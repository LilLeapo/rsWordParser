...
pub struct Document {
    ...
    pub revisions: crate::model::RevisionIndex,
...
impl Document {
    ...
    pub fn rebuild(pkg: &mut Package) -> Result<Document> {
        ...
        let ext_txbx: crate::model::ExtTxbxMap<'_> = txbx_rels
            ...
            .filter_map(|(rid, id)| {
                ...
                Some((
                    rid.clone(),
                    crate::model::ExtTxbxPart { part: *id, dom: d, rels: &pkg.part(*id).rels, idx },
                ))
            })
            .collect();
        ...
        let mut doc = Document {
            ...
            footnotes,
            endnotes,
            revisions: crate::model::RevisionIndex::default(),
            warnings,
        };
    ...
    pub fn refresh_blocks(&mut self, pkg: &mut Package, blocks: &[NodeId]) -> Result<Vec<NodeId>> {
        ...
        for (p, path, sdt, revs) in work {
            ...
            if crate::model::has_external_textbox(&rebuilt) {
                missing.push(p);
            }
...
/// 正文与表格共用的构建器。
pub(in crate::model) struct Builder<'a> {
    pub(in crate::model) dom: &'a Dom,
...
impl<'a> Builder<'a> {
    ...
    fn find_body(&mut self) -> Option<NodeId> {
        let dom = self.dom;
        let root = dom.root();
        if !dom.is(root, build_w(LocalName::Document)) {
            self.warn(root, DiagCode::ModUnparseable, "主 part 根不是 w:document");
            return None;
        }
        let body = dom.semantic_children(root).find(|&n| dom.is(n, build_w(LocalName::Body)));
        if body.is_none() {
    ...
    pub(in crate::model) fn meta(&self, node: NodeId) -> RevisionMeta {
        RevisionMeta {
            node,
    ...
    pub(in crate::model) fn build_container(
        &mut self,
        container: NodeId,
        sdt: Option<&SdtInfo>,
        revs: &[Revision],
            ...
            match class {
                ...
                BodyClass::Sdt => {
                    ...
                    let content = dom
                        ...
                        .find(|&n| dom.is(n, build_w(LocalName::SdtContent)));
                    let before = out.len();
    ...
    pub(in crate::model) fn build_paragraph(
        &mut self,
        p: NodeId,
        sdt: Option<&SdtInfo>,
        revs: &[Revision],
        ...
        let ppr = dom.semantic_children(p).find(|&n| dom.is(n, build_w(LocalName::PPr)));
        ...
        if let Some(num) = &props.num {
            for &n in &num.raw_unmodeled {
                if dom.is(n, build_w(LocalName::NumberingChange)) {
    ...
    fn attach_comments(&self, para: NodeId, inlines: &mut [Inline]) {
        let dom = self.dom;
        let id_of = |n: NodeId| {
            dom.attr_value(n, build_w(LocalName::Id)).map(|v| v.into_owned()).unwrap_or_default()
        };
        ...
        while let Some(n) = stack.pop() {
            ...
            if name == build_w(LocalName::TxbxContent) {
                continue; // 独立内容流
            }
            if name == build_w(LocalName::CommentRangeStart) {
                starts.push(id_of(n));
            } else if name == build_w(LocalName::CommentRangeEnd) {
                ends.push(id_of(n));
            } else if name == build_w(LocalName::CommentReference) {
                has_ref = true;
            }
        ...
        while let Some(n) = stack.pop() {
            ...
            if name == build_w(LocalName::CommentRangeStart) {
                ...
                let id = id_of(n);
                open.retain(|x| *x != id);
            } else if name == build_w(LocalName::R) {
                if !open.is_empty() {
                    let ids: Vec<SpanId> =
                ...
                if let Some(c) = dom
                    .semantic_children(n)
                    .find(|&c| dom.is(c, build_w(LocalName::CommentReference)))
                {
                    refs.push((id_of(c), n));
                }
            }
    ...
    fn preview(&self, node: NodeId) -> String {
        ...
        for n in dom.descendants(node) {
            ...
            if dom.is(n, build_w(LocalName::T)) {
...
fn nodes(dom: &Dom, n: NodeId) -> Vec<LatexItem> {
    dom.content_children(n).map(LatexItem::Node).collect()
}
    ...
    Ok(match item {
        ...
        LatexItem::Node(n) => {
            ...
            Some(match name.local {
                ...
                LocalName::T => {
                    results.push(chars_to_latex(&dom.omml_text_of(n))?);
                    return Ok(None);
                }
                LocalName::F => {
                    // 裸的 noBar 分式只出现在 \binom 的 m:d 包里（那边处理）；别的分式样式在子集之外
                    if dom.prop_val(n, LocalName::FPr, LocalName::Type).is_some_and(|t| t != "bar")
                    {
                        return Err(Error::LatexUnsupported);
                    }
                ...
                LocalName::Func => {
                    let name =
                        dom.plain_text_of_runs(dom.children_named(n, m(LocalName::FName)).next());
                    let name = name.trim();
                ...
                LocalName::LimLow => {
                    if dom.plain_text_of_runs(dom.children_named(n, m(LocalName::E)).next()).trim()
                        != "lim"
                    {
                        return Err(Error::LatexUnsupported);
                    }
        ...
        LatexItem::Node(n) => {
            ...
            match name.local {
                ...
                LocalName::Nary => {
                    ...
                    let sub = if dom.prop_on(n, LocalName::NaryPr, LocalName::SubHide) {
                        String::new()
                    } else {
                        format!("_{{{}}}", p(0))
                    };
                    let sup = if dom.prop_on(n, LocalName::NaryPr, LocalName::SupHide) {
                        String::new()
                    } else {
                        format!("^{{{}}}", p(1))
                    };
...
fn delimiter(dom: &Dom, d: NodeId) -> Result<LatexItem> {
    let beg = dom.prop_val(d, LocalName::DPr, LocalName::BegChr).unwrap_or_else(|| "(".into());
    let end = dom.prop_val(d, LocalName::DPr, LocalName::EndChr).unwrap_or_else(|| ")".into());
    let mut slots = dom.children_named(d, m(LocalName::E));
    let (Some(slot), None) = (slots.next(), slots.next()) else {
        return Err(Error::LatexUnsupported);
    };
    let mut inner = dom.content_children(slot);
    if let (Some(only), None) = (inner.next(), inner.next()) {
        if beg == "("
            && end == ")"
            && dom.is(only, m(LocalName::F))
            && dom.prop_val(only, LocalName::FPr, LocalName::Type).as_deref() == Some("noBar")
        {
...
fn run_to_latex(dom: &Dom, run: NodeId) -> Result<String> {
    let text = dom.run_text(run);
    if !dom.is_plain_run(run) {
...
pub fn to_mathml(dom: &Dom, omath: NodeId) -> String {
    let body = eval_mathml(dom, MathmlItem::Seq(omath));
    if body.is_empty() {
        String::new()
    } else {
...
fn eval_mathml(dom: &Dom, root: MathmlItem) -> String {
    let mut tasks = vec![MathmlTask::Eval(root)];
    let mut results: Vec<String> = Vec::new();
    while let Some(task) = tasks.pop() {
        match task {
            MathmlTask::Eval(item) => {
                let subs = expand_mathml(dom, item, &mut results);
                if let Some(subs) = subs {
                    tasks.push(MathmlTask::Finish(item, subs.len()));
                    tasks.extend(subs.into_iter().rev().map(MathmlTask::Eval));
                }
            }
            MathmlTask::Finish(item, arity) => {
                let at = results.len() - arity;
                let parts: Vec<String> = results.drain(at..).collect();
                results.push(finish_mathml(dom, item, parts));
            }
        }
    }
...
fn finish_mathml(dom: &Dom, item: MathmlItem, parts: Vec<String>) -> String {
    let joined = || parts.concat();
    let p = |i: usize| parts.get(i).map(String::as_str).unwrap_or("");
    match item {
        MathmlItem::Slot(..) | MathmlItem::Row(_) => format!("<mrow>{}</mrow>", joined()),
        MathmlItem::Seq(_) => joined(),
        MathmlItem::Cells(_) => {
            let cells: String = parts.iter().map(|c| format!("<mtd>{c}</mtd>")).collect();
            format!("<mtr>{cells}</mtr>")
        }
        MathmlItem::EqRow(_) => format!("<mtr><mtd>{}</mtd></mtr>", p(0)),
        MathmlItem::Node(n) => {
...
impl<'a> Builder<'a> {
    ...
    pub(in crate::model) fn build_table(
        &mut self,
        tbl: NodeId,
        sdt: Option<&SdtInfo>,
        revs: &[Revision],
        ...
        let tbl_pr = dom.semantic_children(tbl).find(|&n| dom.is(n, table_w(LocalName::TblPr)));
        ...
        if let Some(g) =
            dom.semantic_children(tbl).find(|&n| dom.is(n, table_w(LocalName::TblGrid)))
        {
            for c in dom.semantic_children(g) {
                if dom.is(c, table_w(LocalName::GridCol)) {
                    let mut ctx = crate::semantic::props::Ctx::new(dom, &mut self.warnings);
                    ctx.enter(c);
                    let width = read_attr::<Twips>(c, table_w(LocalName::W), None, &mut ctx);
                    grid.push(GridCol { node: c, w: width });
                } else if dom.is(c, table_w(LocalName::TblGridChange)) {
                    let old = dom
                        .semantic_children(c)
                        .find(|&n| dom.is(n, table_w(LocalName::TblGrid)))
                        .unwrap_or(c);
    ...
    fn collect_rows(&mut self, tbl: NodeId, out: &mut Vec<Row>) {
        let dom = self.dom;
        let mut stack: Vec<TableWrap> = vec![TableWrap { node: tbl, sdt: None, revs: Vec::new() }];
        // 先进后出：一个包裹展开后，它的子节点要按文档序处理，所以逆序压栈
        while let Some(TableWrap { node, sdt, revs }) = stack.pop() {
            if dom.is(node, table_w(LocalName::Tr)) {
    ...
    fn collect_cells(&mut self, tr: NodeId, out: &mut Vec<Cell>) {
        let dom = self.dom;
        let mut stack: Vec<TableWrap> = vec![TableWrap { node: tr, sdt: None, revs: Vec::new() }];
        while let Some(TableWrap { node, sdt, revs }) = stack.pop() {
            if dom.is(node, table_w(LocalName::Tc)) {
        ...
        for child in dom.semantic_children(node) {
            ...
            match name.local {
                LocalName::Tr | LocalName::Tc => {
                    out.push(TableWrap { node: child, sdt: sdt.clone(), revs: revs.to_vec() });
                }
                LocalName::Sdt => {
                    let info = SdtInfo::read(dom, child);
                    if let Some(content) = dom
                        .semantic_children(child)
                        .find(|&n| dom.is(n, table_w(LocalName::SdtContent)))
                    {
                        out.push(TableWrap { node: content, sdt: Some(info), revs: revs.to_vec() });
                    }
                }
                ...
                LocalName::Ins | LocalName::Del | LocalName::MoveFrom | LocalName::MoveTo => {
                    ...
                    let mut inner = revs.to_vec();
                    inner.push(rev);
                    out.push(TableWrap { node: child, sdt: sdt.clone(), revs: inner });
                }
                LocalName::TblPr
                | LocalName::TblGrid
                | LocalName::TrPr
    ...
    fn build_cell(&mut self, tc: NodeId, sdt: Option<&SdtInfo>, revs: &[Revision]) -> Cell {
        let dom = self.dom;
        let tc_pr = dom.semantic_children(tc).find(|&n| dom.is(n, table_w(LocalName::TcPr)));
        ...
        let ends_with_paragraph = blocks.last().is_some_and(|b| {
            dom.is(b.node(), table_w(LocalName::P))

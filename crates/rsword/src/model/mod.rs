...
//
// [`mathml`] 与 [`to_latex`] 是 TS `math.ts` 的 `ommlToMathML` / `ommlToLatex` 的逐字移植——差分按字符串比较，
...
pub mod latex_to_omml {
    ...
    fn parse_control(p: &mut P<'_>) -> Result<String> {
        ...
        if super::is_latex_function(&name) {

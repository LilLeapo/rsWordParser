...
// ===== BEGIN crates/rsword/src/edit/mod.rs =====
...
use crate::semantic::props::CellPropsPatch;
use crate::semantic::props::Change;
use crate::semantic::props::Merge;
use crate::semantic::props::ParaPropsPatch;
use crate::semantic::props::RowPropsPatch;
use crate::semantic::props::RunProps;
use crate::semantic::props::RunPropsPatch;
use crate::semantic::props::SectionPropsPatch;
use crate::semantic::props::SettingsPatch;
use crate::semantic::props::TablePropsPatch;
use crate::semantic::props::TblWidth;
use crate::semantic::props::Val;
use crate::semantic::props::emit_run_props;
use crate::semantic::props::order_index_run_props;
use crate::semantic::props::plan_apply_cell_props_at;
use crate::semantic::props::plan_apply_para_props;
use crate::semantic::props::plan_apply_row_props_at;
use crate::semantic::props::plan_apply_run_props;
use crate::semantic::props::plan_apply_section_props_at;
use crate::span::Affinity;
use crate::span::Anchor;
use crate::span::FieldId;
use crate::span::FieldIndex;
use crate::span::FlowId;
...
use crate::span::field::generate::index as index_gen;
use crate::span::field::generate::index::IndexOptions;
use crate::span::field::generate::seq as seq_gen;
use crate::span::field::generate::toc as toc_gen;
use crate::span::field::generate::toc::TocEntry;
use crate::span::field::generate::toc::TocOptions;
use crate::span::is_content_item;
use crate::span::is_property_element;
use crate::span::plan_save;
use crate::span::plan_update;
use crate::xml::Dirty;
use crate::xml::Dom;
use crate::xml::LocalName;
use crate::xml::NewElement;
use crate::xml::NewNode;
use crate::xml::NodeEdit;
use crate::xml::NodeId;
use crate::xml::NodeKind;
use crate::xml::NsId;
use crate::xml::QName;
use crate::xml::Target;
use crate::xml::dom::Latex;
use crate::xml::dom::Omml;
...
fn atom_ops_w(local: LocalName) -> QName {
...
pub(crate) fn insert_atom(
    ...
    let note_ref = match atom {
        NewAtom::NoteRef { endnote, content } => {
            ...
            result.absorb(upsert_note_entry(s, *endnote, &id, content)?);
            Some((*endnote, id))
        }
        _ => None,
    };
    ...
    let image_run = match atom {
        NewAtom::Image(img) => Some(image_run(s, img)?),
        _ => None,
    };
    ...
    let (parent, before, inherit) = match split_at_public(s, at, loc, &mut result)? {
        Some((left, right)) => {
        ...
        None => {
            ...
            boundary_site_public(s, at.part, at.para, index)?
        }
    };
    ...
    let (run_parent, run_before) = match &mut Tracker::new(s.document(), ctx) {
        ...
        Some(t) => plan_ins_site_public(&mut plan, dom, t, at.para, parent, before)?,
    };
    ...
    match image_run {
        ...
        None => {
            plan.node_edits.push(NodeEdit::Insert {
                parent: run_parent,
                before: run_before,
                node: NewElement::new(atom_ops_w(LocalName::R)),
            });
            // 继承左邻 run 的 `w:rPr`（与 `InsertText` 同一条）
            if let Some(rpr) = inherit.and_then(|r| rpr_of(dom, r)) {
...
fn break_element(kind: BreakKind, clear: Option<&str>) -> NewElement {
    let mut e = NewElement::new(atom_ops_w(LocalName::Br));
    match kind {
        BreakKind::Page => e.push_attr(atom_ops_w(LocalName::Type), "page"),
        BreakKind::Column => e.push_attr(atom_ops_w(LocalName::Type), "column"),
        BreakKind::TextWrapping => {
            if let Some(c) = clear {
                e.push_attr(atom_ops_w(LocalName::Clear), c.to_string());
            }
        }
    }
    e
...
fn symbol_element(font: &str, code: u32) -> NewElement {
    let mut e = NewElement::new(atom_ops_w(LocalName::Sym));
    e.push_attr(atom_ops_w(LocalName::Font), font.to_string());
    e.push_attr(atom_ops_w(LocalName::Char), format!("{:04X}", code & 0xFFFF));
    e
}
fn note_ref_element(endnote: bool, id: &str) -> NewElement {
    ...
    NewElement::new(atom_ops_w(local)).with_attr(atom_ops_w(LocalName::Id), id.to_string())
}
...
pub(crate) fn materialize_at(
    ...
    Ok(match block {
        ...
        // 7.8：块字段与题注
        NewBlock::Field(f) => NewBlock::Many(materialize_field(s, f)?),
        NewBlock::Caption { label, text } => materialize_caption(s, &label, &text, anchor)?,
...
fn insert_chart_parts(
    ...
    let doc_pr_id = chart_ops_next_doc_pr_id(dom);
    let ctx = NamespaceContext::from_dom(dom, flavor);
    // `wp` / `r` 借主 part 的声明（缺了就在 `w:drawing` 上补）；`a` / `c` 与 TS 一样就地声明
    let (wp, wp_decl) = chart_ops_prefix_or_decl(&ctx, NsId::Wp, "wp", NS_WP);
    let (r, r_decl) = chart_ops_prefix_or_decl(&ctx, NsId::R, "r", NS_R);
    let para = format!(
        concat!(
...
pub(crate) fn set_chart_data(
        ...
        match dom.semantic_descendants(root).find(|&n| dom.is(n, c(LocalName::Title))) {
            Some(t) => {
                let mut texts: Vec<NodeId> = dom
                    ...
                    .filter(|&n| dom.is(n, chart_ops_a(LocalName::T)))
                    .collect();
                if texts.is_empty() {
                ...
                if texts.is_empty() {
                    ...
                    for (i, tn) in texts.into_iter().enumerate() {
                        ...
                        set_segment_text(dom, tn, text, &mut plan);
                    }
                }
            }
    ...
    for (i, ser) in sers.into_iter().enumerate() {
        ...
        if let Some(sp) = sp {
            ...
            {
                set_segment_text(dom, v, name, &mut plan);
            }
...
fn point_edits(dom: &Dom, container: NodeId, texts: &[Option<String>], plan: &mut MutationPlan) {
    for pt in dom.semantic_descendants(container).filter(|&n| dom.is(n, c(LocalName::Pt))) {
        ...
        if let Some(v) = dom.semantic_descendants(pt).find(|&n| dom.is(n, c(LocalName::V))) {
            set_segment_text(dom, v, text, plan);
        }
    }
}
...
fn new_title(dom: &Dom, root: NodeId, text: &str, plan: &mut MutationPlan) {
    ...
    let title =
        NewElement::new(c(LocalName::Title))
            .with_child(
                NewElement::new(c(LocalName::Tx)).with_child(
                    NewElement::new(c(LocalName::Rich))
                        .with_child(NewElement::new(chart_ops_a(LocalName::BodyPr)))
                        .with_child(NewElement::new(chart_ops_a(LocalName::LstStyle)))
                        .with_child(NewElement::new(chart_ops_a(LocalName::P)).with_child(
                            NewElement::new(chart_ops_a(LocalName::R)).with_child(
                                NewElement::new(chart_ops_a(LocalName::T)).with_text(text),
                            ),
                        )),
                ),
            )
...
fn inject_title(dom: &Dom, title: NodeId, text: &str, plan: &mut MutationPlan) {
    let run = NewElement::new(chart_ops_a(LocalName::R))
        .with_child(NewElement::new(chart_ops_a(LocalName::T)).with_text(text));
    let tx = child(dom, title, c(LocalName::Tx));
    let p = tx.and_then(|tx| {
        dom.semantic_descendants(tx).find(|&n| dom.is(n, chart_ops_a(LocalName::P)))
    });
    if let Some(p) = p {
        // Word 的自动标题带一个只有 a:endParaRPr 的空 c:tx/c:rich 段落：run 按 schema 顺序插在它之前
        let end_pr = child(dom, p, chart_ops_a(LocalName::EndParaRPr));
        plan.node_edits.push(NodeEdit::Insert {
    ...
    let rich = NewElement::new(c(LocalName::Tx)).with_child(
        NewElement::new(c(LocalName::Rich))
            .with_child(NewElement::new(chart_ops_a(LocalName::BodyPr)))
            .with_child(NewElement::new(chart_ops_a(LocalName::LstStyle)))
            .with_child(NewElement::new(chart_ops_a(LocalName::P)).with_child(run)),
    );
    match tx {
...
fn drawing_ops_w(local: LocalName) -> QName {
    QName::new(NsId::W, local)
}
...
fn drawing_ops_live_children(dom: &Dom, n: NodeId) -> impl Iterator<Item = NodeId> + '_ {
...
fn shell(dom: &Dom, drawing: NodeId) -> Result<NodeId> {
    drawing_ops_live_children(dom, drawing)
...
fn require_drawing(s: &EditSession, drawing: NodeId) -> Result<()> {
    ...
    if (drawing.0 as usize) >= dom.node_count()
        ...
        || !dom.is(drawing, drawing_ops_w(LocalName::Drawing))
    {
...
pub(crate) fn set_geometry(
    ...
    let mut plan = MutationPlan::new(part);
    if let Some(p) = dom.ancestors(drawing).find(|&x| dom.is(x, drawing_ops_w(LocalName::P))) {
        plan.touch(p);
    }
    ...
    if let Some((cx, cy)) = geom.extent_emu {
        ...
        if let Some(ext) =
            drawing_ops_live_children(dom, shell).find(|&c| dom.is(c, wp(LocalName::Extent)))
        {
        ...
        if let Some(ee) =
            drawing_ops_live_children(dom, shell).find(|&c| dom.is(c, wp(LocalName::EffectExtent)))
        {
            for (n, v) in
    ...
    if let Some((x, y)) = geom.pos_offset_emu {
        for (which, v) in [(LocalName::PositionH, x), (LocalName::PositionV, y)] {
            let Some(pos) = drawing_ops_live_children(dom, shell).find(|&c| dom.is(c, wp(which)))
            else {
                continue;
            };
            match drawing_ops_live_children(dom, pos).find(|&c| dom.is(c, wp(LocalName::PosOffset)))
            {
                Some(off) => set_segment_text(dom, off, &v.to_string(), &mut plan),
    ...
    if let Some(crop) = geom.crop {
        for fill in blip_fills(dom, drawing) {
            let existing = drawing_ops_live_children(dom, fill)
                .find(|&c| dom.is(c, drawing_ops_a(LocalName::SrcRect)));
            match (crop, existing) {
                (None, Some(n)) => plan.node_edits.push(NodeEdit::Delete(n)),
                (None, None) => {}
                (Some(r), old) => {
                    let mut e = NewElement::new(drawing_ops_a(LocalName::SrcRect));
                    let none = |l: LocalName| QName::new(NsId::None, l);
                    for (n, v) in [
                        (LocalName::L, r.l),
                        (LocalName::T, r.t),
                    ...
                    match old {
                        ...
                        None => plan.node_edits.push(NodeEdit::Insert {
                            ...
                            before: drawing_ops_live_children(dom, fill)
                                .find(|&c| !dom.is(c, drawing_ops_a(LocalName::Blip))),
                            node: e,
                        }),
                    }
                }
...
fn xfrms(dom: &Dom, drawing: NodeId) -> Vec<NodeId> {
    dom.descendants(drawing)
        ...
        .filter(|&n| dom.is(n, drawing_ops_a(LocalName::Xfrm)))
        .collect()
}
/// 每个 `a:xfrm` 下的 `a:ext`。
fn xfrm_exts(dom: &Dom, drawing: NodeId) -> Vec<NodeId> {
    xfrms(dom, drawing)
        .into_iter()
        .filter_map(|x| {
            drawing_ops_live_children(dom, x).find(|&c| dom.is(c, drawing_ops_a(LocalName::Ext)))
        })
        .collect()
}
fn blip_fills(dom: &Dom, drawing: NodeId) -> Vec<NodeId> {
    dom.descendants(drawing)
        .filter(|&n| dom.node(n).dirty != Dirty::Deleted)
        .filter(|&n| {
            dom.is(n, drawing_ops_a(LocalName::BlipFill))
...
pub(crate) fn set_shape_style(
    ...
    let sp_pr = if dom.is(shape, QName::new(NsId::Wps, LocalName::SpPr)) {
        ...
        drawing_ops_live_children(dom, shape)
            .find(|&c| dom.is(c, QName::new(NsId::Wps, LocalName::SpPr)))
            .ok_or_else(|| Error::edit(DiagCode::EditBadPosition, "形状里没有 wps:spPr"))?
    };
    let mut plan = MutationPlan::new(part);
    if let Some(p) = dom.ancestors(sp_pr).find(|&x| dom.is(x, drawing_ops_w(LocalName::P))) {
        plan.touch(p);
    }
    if let Some(f) = fill {
        // `a:solidFill` / `a:noFill` 在 `a:prstGeom` 之后、`a:ln` 之前（CT_ShapeProperties）
        let before = drawing_ops_live_children(dom, sp_pr)
            .find(|&c| dom.is(c, drawing_ops_a(LocalName::Ln)));
        replace_fill(dom, &mut plan, sp_pr, before, f.as_deref());
    }
    if let Some(o) = outline {
        let ln = drawing_ops_live_children(dom, sp_pr)
            .find(|&c| dom.is(c, drawing_ops_a(LocalName::Ln)));
        match ln {
            Some(ln) => replace_fill(dom, &mut plan, ln, None, o.as_deref()),
            None => {
                let mut e = NewElement::new(drawing_ops_a(LocalName::Ln));
...
fn replace_fill(
    ...
    let old: Vec<NodeId> = drawing_ops_live_children(dom, container)
        .filter(|&c| {
            dom.is(c, drawing_ops_a(LocalName::SolidFill))
                || dom.is(c, drawing_ops_a(LocalName::NoFill))
                || dom.is(c, drawing_ops_a(LocalName::GradFill))
                || dom.is(c, drawing_ops_a(LocalName::BlipFill))
        })
        .collect();
...
fn fill_element(color: Option<&str>) -> NewElement {
    match color {
        Some(rgb) => NewElement::new(drawing_ops_a(LocalName::SolidFill)).with_child(
            NewElement::new(drawing_ops_a(LocalName::SrgbClr))
                .with_attr(QName::new(NsId::None, LocalName::Val), rgb.trim_start_matches('#')),
        ),
        None => NewElement::new(drawing_ops_a(LocalName::NoFill)),
    }
}
...
fn in_place_anchor(
    ...
    plan: &mut MutationPlan,
    anchor: NodeId,
    wrap: media_ops::ImageWrap,
    pos: Option<&AnchorPos>,
    z_order: Option<i64>,
) {
    use crate::edit::media_ops::ImageWrap;
    let none = |l: LocalName| QName::new(NsId::None, l);
    plan.node_edits.push(NodeEdit::SetAttr {
    ...
    let old_wrap = drawing_ops_live_children(dom, anchor).find(|&c| is_wrap_element(dom, c));
    let polygon = old_wrap.filter(|_| keeps_polygon(dom, old_wrap, wrap)).and_then(|w| {
        drawing_ops_live_children(dom, w).find(|&c| dom.is(c, wp(LocalName::WrapPolygon)))
    });
    let before = old_wrap.or_else(|| {
        drawing_ops_live_children(dom, anchor).find(|&c| dom.is(c, wp(LocalName::DocPr)))
    });
    ...
    let Some(p) = pos else {
        ...
        if let Some(align) = drawing_ops_live_children(dom, anchor)
            .find(|&c| dom.is(c, wp(LocalName::PositionH)))
            .and_then(|h| {
                drawing_ops_live_children(dom, h).find(|&c| dom.is(c, wp(LocalName::Align)))
            })
        {
            set_segment_text(dom, align, default_align(wrap), plan);
        }
        return;
    };
    for (which, axis) in [(LocalName::PositionH, &p.h), (LocalName::PositionV, &p.v)] {
        let e = position_element(which, axis);
        match drawing_ops_live_children(dom, anchor).find(|&c| dom.is(c, wp(which))) {
            Some(old) => plan.node_edits.push(NodeEdit::Replace { old, node: e }),
            None => plan.node_edits.push(NodeEdit::Insert {
                parent: Target::Node(anchor),
                before: drawing_ops_live_children(dom, anchor)
                    .find(|&c| !dom.is(c, wp(LocalName::SimplePos))),
                node: e,
            }),
        }
    }
...
fn keeps_polygon(dom: &Dom, old_wrap: Option<NodeId>, next: media_ops::ImageWrap) -> bool {
    use crate::edit::media_ops::ImageWrap;
...
fn rebuild_shell(
    ...
    let carry = |plan: &mut MutationPlan, upto: usize| {
        for &(ns, local) in &CARRIED[..upto] {
            if let Some(n) =
                drawing_ops_live_children(dom, old).find(|&c| dom.is(c, QName::new(ns, local)))
            {
...
fn default_pos(wrap: media_ops::ImageWrap) -> AnchorPos {
    AnchorPos {
        h: AnchorAxis {
            relative_from: "column".into(),
            pos: AxisPos::Align(default_align(wrap).into()),
        },
...
fn wrap_element(wrap: media_ops::ImageWrap, fresh_polygon: bool) -> NewElement {
    use crate::edit::media_ops::ImageWrap;
    let none = |l: LocalName| QName::new(NsId::None, l);
...
pub fn next_seq_number(s: &EditSession, label: &str, at: BlockAt) -> u32 {
...
fn ensure_toc_bookmarks(s: &mut EditSession, entries: &mut [TocEntry]) -> Result<MutationResult> {
    ...
    for (e, para) in entries.iter_mut().zip(paras) {
        ...
        let len = s
            ...
            .unwrap_or(0);
        out.absorb(add_bookmark(s, &name, InlinePos::new(para, 0), InlinePos::new(para, len))?);
...
pub(crate) fn materialize_caption(
    ...
    let n = match at {
        ...
        None => next_seq_number(s, label, BlockAt::End(s.dom().root())),
    };
...
pub(crate) fn regenerate(
    ...
    result.absorb(update_block_field(s, field, blocks, ctx)?);
...
pub(crate) const NS_PIC: &str = "http://schemas.openxmlformats.org/drawingml/2006/picture";
...
impl EditSession {
    ...
    pub(crate) fn insert_ink(&mut self, para: NodeId, ink: &NewInk) -> Result<MutationResult> {
        ...
        let id = media_ops_next_doc_pr_id(dom);
        let ctx = NamespaceContext::from_dom(dom, flavor);
        let (wp, wp_decl) = media_ops_prefix_or_decl(&ctx, NsId::Wp, "wp", NS_WP);
        let (r, r_decl) = media_ops_prefix_or_decl(&ctx, NsId::R, "r", NS_R);
...
fn inline_w(local: LocalName) -> QName {
    QName::w(local)
}
...
fn text_element(deleted: bool, text: &str) -> NewElement {
    NewElement::new(inline_w(if deleted { LocalName::DelText } else { LocalName::T }))
    ...
    for c in text.chars() {
        match c {
            '\t' => {
                ...
                out.push(NewElement::new(inline_w(LocalName::Tab)));
            }
            '\n' => {
                flush(&mut buf, &mut out);
                out.push(NewElement::new(inline_w(LocalName::Br)));
            }
            '\u{0C}' => {
                flush(&mut buf, &mut out);
                out.push(
                    NewElement::new(inline_w(LocalName::Br))
                        .with_attr(inline_w(LocalName::Type), "page"),
                );
            }
            '\u{0B}' => {
                flush(&mut buf, &mut out);
                out.push(
                    NewElement::new(inline_w(LocalName::Br))
                        .with_attr(inline_w(LocalName::Type), "column"),
                );
            }
            '\r' => {
                flush(&mut buf, &mut out);
                out.push(NewElement::new(inline_w(LocalName::Cr)));
            }
            _ => buf.push(c),
        }
    }
...
pub fn new_run(text: &str, props: Option<NewElement>, deleted: bool) -> NewElement {
    let mut r = NewElement::new(inline_w(LocalName::R));
    if let Some(p) = props {
        r.push_child(p);
    }
...
fn marker(m: &NewMarker) -> NewElement {
    match m {
        NewMarker::BookmarkStart { id, name } => {
            NewElement::new(inline_w(LocalName::BookmarkStart))
                .with_attr(inline_w(LocalName::Id), id.clone())
                .with_attr(inline_w(LocalName::Name), name.clone())
        }
        NewMarker::BookmarkEnd { id } => NewElement::new(inline_w(LocalName::BookmarkEnd))
            .with_attr(inline_w(LocalName::Id), id.clone()),
        NewMarker::CommentRangeStart { id } => {
            NewElement::new(inline_w(LocalName::CommentRangeStart))
                .with_attr(inline_w(LocalName::Id), id.clone())
        }
        NewMarker::CommentRangeEnd { id } => NewElement::new(inline_w(LocalName::CommentRangeEnd))
            .with_attr(inline_w(LocalName::Id), id.clone()),
        NewMarker::CommentReference { id } => NewElement::new(inline_w(LocalName::R)).with_child(
            NewElement::new(inline_w(LocalName::CommentReference))
                .with_attr(inline_w(LocalName::Id), id.clone()),
        ),
    }
}
...
impl Emitter {
    ...
    fn revision_attrs(&mut self, e: &mut NewElement, rev: &NewRevision) {
        ...
        e.push_attr(inline_w(LocalName::Id), id);
        e.push_attr(inline_w(LocalName::Author), rev.author.clone());
        if let Some(d) = &rev.date {
            e.push_attr(inline_w(LocalName::Date), d.clone());
        }
    }
    ...
    pub fn emit(&mut self, inline: &NewInline, deleted: bool, out: &mut Vec<NewElement>) {
        match inline {
            ...
            NewInline::Hyperlink { target, tooltip, inlines } => {
                let mut h = NewElement::new(inline_w(LocalName::Hyperlink));
                match target {
                    NewLinkTarget::Rel(rid) => {
                        h.push_attr(QName::new(NsId::R, LocalName::Id), rid.clone())
                    }
                    NewLinkTarget::Anchor(a) => h.push_attr(inline_w(LocalName::Anchor), a.clone()),
                }
                if let Some(t) = tooltip {
                    h.push_attr(inline_w(LocalName::Tooltip), t.clone());
                }
            ...
            NewInline::Ins { rev, inlines } => {
                let mut e = NewElement::new(inline_w(LocalName::Ins));
            ...
            NewInline::Del { rev, inlines } => {
                let mut e = NewElement::new(inline_w(LocalName::Del));
                self.revision_attrs(&mut e, rev);
                let mut kids = Vec::new();
                for i in inlines {
            ...
            NewInline::Field { instr, result, separate, dirty, props } => {
                let fld = |kind: &str, dirty: bool| {
                    let mut e = NewElement::new(inline_w(LocalName::FldChar))
                        .with_attr(inline_w(LocalName::FldCharType), kind);
                    if dirty {
                        e.push_attr(inline_w(LocalName::Dirty), "true");
                    }
                    e
                };
                ...
                out.push(structural(
                    NewElement::new(inline_w(LocalName::InstrText))
                        .with_attr(QName::new(NsId::Xml, LocalName::Space), "preserve")
    ...
    pub(crate) fn replace_image_media(
        ...
        if let Some(mut t) = Tracker::new(self.document(), ctx) {
            ...
            plan.node_edits.push(NodeEdit::Insert {
                ...
                before: next_element_sibling(dom, run),
...
pub(crate) fn media_ops_prefix_or_decl(
    ctx: &NamespaceContext,
    ns: NsId,
    default: &str,
    uri: &str,
...
pub(crate) fn media_ops_next_doc_pr_id(dom: &Dom) -> i64 {
    let mut max = 0i64;
...
fn note_ops_w(local: LocalName) -> QName {
    QName::new(NsId::W, local)
}
...
pub(crate) fn set_note_content(
    ...
    upsert_note_entry(s, endnote, id, &paras)
}
...
pub(crate) fn drop_references(
    ...
    while let Some(n) = stack.pop() {
        ...
        let Some(e) = dom.element(n) else { continue };
        stack.extend(e.children.iter().rev());
        if !dom.is(n, note_ops_w(refname))
            || dom.attr_value(n, note_ops_w(LocalName::Id)).as_deref() != Some(id)
        {
            continue;
        }
        let Some(run) = dom.parent(n).filter(|&r| dom.is(r, note_ops_w(LocalName::R))) else {
            plan.node_edits.push(NodeEdit::Delete(n));
            continue;
        };
        ...
        let others = dom
            ...
            .filter(|&c| c != n && !dom.is(c, note_ops_w(LocalName::RPr)))
            .count();
        let victim: NodeId = if others == 0 { run } else { n };
        plan.node_edits.push(NodeEdit::Delete(victim));
        if let Some(p) = dom.ancestors(victim).find(|&a| dom.is(a, note_ops_w(LocalName::P))) {
            plan.touch(p);
        }
    }
    if plan.is_empty() {
...
fn dispatch(s: &mut EditSession, op: EditOp, ctx: &EditContext) -> Result<MutationResult> {
    match op {
        ...
        EditOp::SetSectionProps { sect, patch } => set_section_props(s, sect, &patch, ctx),
        EditOp::SetHeaderFooter { sect, kind, variant, content } => {
            set_header_footer(s, sect, kind, variant, content, ctx)
        }
        EditOp::LinkHeaderFooter { sect, kind, variant, part } => {
            link_header_footer(s, sect, kind, variant, part)
        }
        EditOp::SetWatermark { sect, text } => set_watermark(s, sect, text),
        EditOp::SetPageColor { color } => set_page_color(s, color),
        EditOp::SetDocumentSettings { patch } => set_document_settings(s, &patch),
        EditOp::SetChartData { part, patch } => set_chart_data(s, part, &patch),
        ...
        EditOp::InsertAtom { at, atom } => insert_atom(s, at, &atom, ctx),
        EditOp::SetNoteContent { endnote, id, content } => {
            set_note_content(s, endnote, &id, &content)
        }
        EditOp::RemoveNote { endnote, id } => remove_note(s, endnote, &id),
        EditOp::SetSdtContent { sdt, inlines } => set_sdt_content(s, sdt, &inlines, ctx),
        EditOp::RemoveSdtShell { sdt } => remove_sdt_shell(s, sdt),
        EditOp::SetMathTokens { math, tokens } => set_math_tokens(s, math, &tokens),
        EditOp::SetDrawingGeometry { drawing, geom } => set_geometry(s, drawing, &geom),
        EditOp::SetDrawingZOrder { drawing, z } => set_z_order(s, drawing, z),
        EditOp::SetDrawingWrap { drawing, wrap, pos, z_order } => {
            set_wrap(s, drawing, wrap, pos.as_ref(), z_order)
        }
        EditOp::SetShapeStyle { shape, fill, outline } => set_shape_style(s, shape, fill, outline),
        EditOp::SetTextboxContent { textbox, blocks } => {
            set_textbox_content(s, textbox, blocks, ctx)
        }
        EditOp::RegenerateBlockField { field, options } => regenerate(s, field, options, ctx),
        EditOp::InsertSectionBreak { after, kind } => insert_section_break(s, after, kind, ctx),
        EditOp::DeleteSectionBreak { sect } => delete_section_break(s, sect, ctx),
        EditOp::AcceptRevision { rev } => one(s, rev, true),
        EditOp::RejectRevision { rev } => one(s, rev, false),
        EditOp::AcceptAll { author } => all(s, author.as_deref(), true),
        EditOp::RejectAll { author } => all(s, author.as_deref(), false),
    }
}
...
// ---- 小工具 ----------------------------------------------------------------------------------
fn ops_w(local: LocalName) -> QName {
    QName::w(local)
}
...
fn ops_live_children(dom: &Dom, n: NodeId) -> impl Iterator<Item = NodeId> + '_ {
    dom.children(n).iter().copied().filter(move |&c| dom.node(c).dirty != Dirty::Deleted)
}
fn ops_child_named(dom: &Dom, parent: NodeId, name: QName) -> Option<NodeId> {
    ops_live_children(dom, parent).find(|&c| dom.is(c, name))
}
pub(crate) fn rpr_of(dom: &Dom, run: NodeId) -> Option<NodeId> {
    ops_child_named(dom, run, ops_w(LocalName::RPr))
}
pub(crate) fn ppr_of(dom: &Dom, para: NodeId) -> Option<NodeId> {
    ops_child_named(dom, para, ops_w(LocalName::PPr))
}
/// 下一个未删除的兄弟。
...
fn sole_text_child(dom: &Dom, t: NodeId) -> Option<NodeId> {
    let mut it = ops_live_children(dom, t);
    let first = it.next()?;
    if it.next().is_some() {
        return None;
    }
...
pub(crate) fn next_revision_id(dom: &Dom) -> u32 {
    ...
    for n in dom.descendants(dom.root()) {
        ...
        if let Some(v) = dom.attr_value(n, ops_w(LocalName::Id))
...
fn split_run(
    ...
    plan.node_edits.push(NodeEdit::Insert {
        ...
        node: NewElement::new(ops_w(LocalName::R)),
    });
...
fn plan_ins_site(
    ...
    match track_site_of(dom, parent, para, &t.author) {
        ...
        TrackSite::OtherIns(ins) => {
            ...
            let kids: Vec<NodeId> = ops_live_children(dom, ins).collect();
...
fn delete_range(
    ...
    let notes = covered_note_refs(dom, tb, a, b);
    let mut result = s.commit_plan(plan)?;
    for (endnote, id) in notes {
        result.absorb(drop_references(s, endnote, &id)?);
        result.absorb(remove_note_entry(s, endnote, &id)?);
    }
    Ok(result)
}
...
fn covered_note_refs(dom: &Dom, tb: &TextBlock, a: u32, b: u32) -> Vec<(bool, String)> {
    ...
    for (inline, span) in tb.inlines.iter().zip(InlinePos::inline_spans(tb)) {
        ...
        for n in dom.descendants(node) {
            ...
            let endnote = if dom.is(n, ops_w(LocalName::FootnoteReference)) {
                false
            } else if dom.is(n, ops_w(LocalName::EndnoteReference)) {
                true
            } else {
                continue;
            };
            if let Some(id) = dom.attr_value(n, ops_w(LocalName::Id)) {
...
fn delete_range_tracked(
    ...
    for off in [to.offset, from.offset] {
        let loc = off.locate(text_block(s, from.part, from.para)?)?;
    ...
    for &n in &dropped {
        if let TrackSite::OwnIns(ins) = track_site_of(dom, n, from.para, &t.author)
            ...
            && ops_live_children(dom, ins).all(|c| dropped.contains(&c))
        {
    ...
    match rpr_of(dom, run) {
        ...
        None => {
            ...
            plan.node_edits.push(NodeEdit::Insert {
                ...
                before: ops_live_children(dom, run).next(),
                node: NewElement::new(ops_w(LocalName::RPr)),
            });
            let change =
                t.marker(LocalName::RPrChange).with_child(NewElement::new(ops_w(LocalName::RPr)));
            plan.node_edits.push(NodeEdit::Insert {
                parent: Target::New(k),
                before: None,
                node: change,
...
fn plan_para_props_change(
    ...
    match ppr_of(dom, para) {
        ...
        None => {
            ...
            let before = ops_live_children(dom, para).next();
            let k = plan.node_edits.len();
            plan.node_edits.push(NodeEdit::Insert {
                parent: Target::Node(para),
                before,
                node: NewElement::new(ops_w(LocalName::PPr)),
            });
            let change =
                t.marker(LocalName::PPrChange).with_child(NewElement::new(ops_w(LocalName::PPr)));
            plan.node_edits.push(NodeEdit::Insert {
                parent: Target::New(k),
                before: None,
                node: change,
...
fn replace_container_inlines_in(
    ...
    for c in ops_live_children(dom, container) {
        if !dom.is(c, ops_w(LocalName::PPr)) {
...
fn set_math_tokens(s: &mut EditSession, math: NodeId, tokens: &[String]) -> Result<MutationResult> {
    ...
    let mut plan = MutationPlan::new(part);
    if let Some(p) = dom.ancestors(math).find(|&a| dom.is(a, ops_w(LocalName::P))) {
        plan.touch(p);
    }
...
fn replace_para_props(
    ...
    let mut plan = MutationPlan::new(s.part_or_main(part));
    plan.touch(para);
    let first = ops_live_children(dom, para).next();
    let old_ppr = ppr_of(dom, para);
    let change = old_ppr
        .filter(|_| tracked)
        .and_then(|p| ops_live_children(dom, p).find(|&c| dom.is(c, ops_w(LocalName::PPrChange))));
    let k = plan.node_edits.len();
    match (props, change) {
    ...
    for c in ops_live_children(dom, para) {
        if dom.is(c, ops_w(LocalName::PPr)) {
            plan.node_edits.push(NodeEdit::Delete(c));
        }
    }
...
fn block_site(dom: &Dom, at: BlockAt) -> Result<(NodeId, Option<NodeId>)> {
    ...
    match at {
        ...
        BlockAt::End(c) => {
            ...
            let before = last.filter(|&l| dom.is(l, ops_w(LocalName::SectPr)));
            Ok((c, before))
        }
    }
}
pub(super) fn new_block_element(dom: &Dom, block: NewBlock) -> NewElement {
    match block {
        ...
        NewBlock::Paragraph { props, inlines } => {
            let mut p = NewElement::new(ops_w(LocalName::P));
        ...
        NewBlock::Wrapped { mut wrapper, block } => {
            ...
            if let Some(id) =
                wrapper.attrs.iter_mut().find(|(n, v)| *n == ops_w(LocalName::Id) && v.is_empty())
            {
...
fn owning_table(dom: &Dom, node: NodeId) -> Option<NodeId> {
    std::iter::once(node).chain(dom.ancestors(node)).find(|&n| dom.is(n, ops_w(LocalName::Tbl)))
}
...
fn props_site(dom: &Dom, parent: NodeId, container: LocalName) -> (Option<NodeId>, Option<NodeId>) {
    ...
    let existing = kids.iter().copied().find(|&c| dom.is(c, ops_w(container)));
    let before = if container == LocalName::TrPr {
        kids.iter().copied().find(|&c| !dom.is(c, ops_w(LocalName::TblPrEx)))
    } else {
        kids.first().copied()
    };
    (existing, before)
...
fn insert_block(
    ...
    let block = materialize_at(s, block, Some(at.at))?;
...
pub(super) fn plan_delete_block_tracked(
    ...
    } else if dom.is(node, ops_w(LocalName::Tbl)) {
        plan.structure_changed = true;
        for row in ops_live_children(dom, node).collect::<Vec<_>>() {
            if !dom.is(row, ops_w(LocalName::Tr)) {
                continue;
            }
...
fn move_block(
    ...
    if before.is_none() && !dom.is(node, ops_w(LocalName::P)) {
        keep_cell_paragraph(dom, parent, None, &mut plan);
    }
    s.commit_plan(plan)
}
...
pub(super) fn keep_cell_paragraph(
    ...
    if !dom.is(container, ops_w(LocalName::Tc)) {
        return;
    }
    let last = dom
        .children(container)
    ...
    if last.is_some_and(|n| dom.is(n, ops_w(LocalName::P))) {
        return;
    }
    plan.structure_changed = true;
    plan.node_edits.push(NodeEdit::Insert {
        parent: Target::Node(container),
        before: None,
        node: NewElement::new(ops_w(LocalName::P)),
    });
}
...
fn comment_reference_run(id: &str) -> NewElement {
    let rpr = NewElement::new(ops_w(LocalName::RPr)).with_child(
        NewElement::new(ops_w(LocalName::RStyle))
            .with_attr(ops_w(LocalName::Val), "CommentReference"),
    );
    NewElement::new(ops_w(LocalName::R)).with_child(rpr).with_child(
        NewElement::new(ops_w(LocalName::CommentReference)).with_attr(ops_w(LocalName::Id), id),
    )
}
...
fn entry_run(r: &NewRun) -> NewElement {
    let mut e = NewElement::new(ops_w(LocalName::R));
...
fn annotation_ref_run() -> NewElement {
    ...
    NewElement::new(ops_w(LocalName::R))
        .with_child(rpr)
        .with_child(NewElement::new(ops_w(LocalName::AnnotationRef)))
}
...
fn add_comment(
    ...
    let mut entry =
        NewElement::new(ops_w(LocalName::Comment)).with_attr(ops_w(LocalName::Id), id.clone());
    entry.push_attr(ops_w(LocalName::Author), c.author.clone());
    if let Some(i) = &c.initials {
        entry.push_attr(ops_w(LocalName::Initials), i.clone());
    }
    if let Some(d) = &c.date {
        entry.push_attr(ops_w(LocalName::Date), d.clone());
    }
    ...
    let marker = |local: LocalName| {
        NewElement::new(ops_w(local)).with_attr(ops_w(LocalName::Id), id.clone())
    };
    let mut plan = MutationPlan::new(part);
    plan.touch(from.para);
    plan.node_edits.push(NodeEdit::Insert {
...
fn remove_comment(s: &mut EditSession, id: &str) -> Result<MutationResult> {
    ...
    if !victims.is_empty() {
        let mut plan = MutationPlan::new(part);
        for n in &victims {
            if let Some(p) = dom.ancestors(*n).find(|&a| dom.is(a, ops_w(LocalName::P))) {
                plan.touch(p);
            }
            plan.node_edits.push(NodeEdit::Delete(*n));
        }
    ...
    let (node, para_id, first_rpr) = {
        ...
        let rpr = dom.and_then(|d| {
            c.rich
                ...
                .flatten()
                .next()
                .and_then(|r| d.semantic_children(r.node).find(|&n| d.is(n, ops_w(LocalName::RPr))))
...
pub(crate) fn upsert_comment_entry(
    ...
    match existing {
        Some((node, _)) => {
            ...
            for (name, value) in [
                ...
                match value {
                    ...
                    None => plan
                        .node_edits
                        .push(NodeEdit::RemoveAttr { node: Target::Node(node), name: ops_w(name) }),
                }
            }
            for p in body {
                plan.node_edits.push(NodeEdit::Insert {
        ...
        None => {
            ...
            let root = dom.root();
            let mut entry =
                NewElement::new(ops_w(LocalName::Comment)).with_attr(ops_w(LocalName::Id), id);
            if let Some(a) = author {
                entry.push_attr(ops_w(LocalName::Author), a);
            }
            if let Some(i) = initials {
                entry.push_attr(ops_w(LocalName::Initials), i);
            }
            if let Some(d) = date {
                entry.push_attr(ops_w(LocalName::Date), d);
            }
            for p in body {
                entry.push_child(p);
            }
...
pub(crate) fn upsert_note_entry(
    ...
    match existing {
        Some(node) => {
            ...
            let ref_run = dom
                .descendants(node)
                .find(|&n| dom.is(n, ops_w(ref_name)))
                .and_then(|m| dom.ancestors(m).find(|&a| dom.is(a, ops_w(LocalName::R))));
            let lead = ref_run
        ...
        None => {
            let root = dom.root();
            let lead =
                NewElement::new(ops_w(LocalName::R)).with_child(NewElement::new(ops_w(ref_name)));
            let mut entry = NewElement::new(ops_w(entry_name)).with_attr(ops_w(LocalName::Id), id);
...
fn merge_with_next(
    ...
    let next = next_element_sibling(dom, para)
        .filter(|&n| dom.is(n, ops_w(LocalName::P)))
...
pub(super) fn plan_merge_with_next(
    ...
    let part = s.part_or_main(at);
    let dom = s.dom_in(at)?;
    let Some(next) = next_element_sibling(dom, para).filter(|&n| dom.is(n, ops_w(LocalName::P)))
    else {
        return Ok(None);
    };
...
fn next_bookmark_id(s: &mut EditSession) -> u32 {
    ...
    for n in dom.descendants(dom.root()) {
        ...
        let is_marker =
            dom.is(n, ops_w(LocalName::BookmarkStart)) || dom.is(n, ops_w(LocalName::BookmarkEnd));
        if is_marker
            && let Some(v) = dom.attr_value(n, ops_w(LocalName::Id))
            && let Ok(id) = v.trim().parse::<u32>()
        {
            max = max.max(id);
        }
...
pub(super) fn add_bookmark(
    ...
    plan.node_edits.push(NodeEdit::Insert {
        parent: Target::Node(from.para),
        before: start_before,
        node: NewElement::new(ops_w(LocalName::BookmarkStart))
            .with_attr(ops_w(LocalName::Id), id.clone())
            .with_attr(ops_w(LocalName::Name), name),
    });
    plan.node_edits.push(NodeEdit::Insert {
        parent: Target::Node(to.para),
        before: end_before,
        node: NewElement::new(ops_w(LocalName::BookmarkEnd))
            .with_attr(ops_w(LocalName::Id), id.clone()),
    });
    let r = s.commit_plan(plan)?;
...
fn set_link_target(
    ...
    match link {
        LinkRef::Field(id) => set_field_link_target(s, id, target, ctx),
        // `w:hyperlink` 的 `r:id` / `w:anchor` 是元素属性，Word 不把它记成修订
        LinkRef::Element(node) => set_hyperlink_target(s, node, target),
    }
}
...
fn set_hyperlink_target(
    ...
    let dom = s.dom();
    let mut plan = MutationPlan::new(part);
    if let Some(p) = dom.ancestors(node).find(|&a| dom.is(a, ops_w(LocalName::P))) {
        plan.touch(p);
    }
...
fn set_field_link_target(
    ...
    let mut plan = MutationPlan::new(part);
    if let Some(p) = dom.ancestors(instr_nodes[0]).find(|&a| dom.is(a, ops_w(LocalName::P))) {
        plan.touch(p);
    }
    ...
    if let Some(mut t) = Tracker::new(s.document(), ctx) {
        let has_instr = instr_nodes
            .iter()
            .any(|&r| dom.semantic_children(r).any(|c| dom.is(c, ops_w(LocalName::InstrText))));
        if !has_instr {
        ...
        let mut run = NewElement::new(ops_w(LocalName::R));
        ...
        run.push_child(
            NewElement::new(ops_w(LocalName::InstrText))
                .with_attr(QName::new(NsId::Xml, LocalName::Space), "preserve")
                .with_text(text),
        );
    ...
    for run in &instr_nodes {
        for seg in dom.semantic_children(*run).collect::<Vec<_>>() {
            if !dom.is(seg, ops_w(LocalName::InstrText)) {
                continue;
            }
...
fn toggle_checkbox(s: &mut EditSession, id: crate::span::FieldId) -> Result<MutationResult> {
    ...
    let want = !checked;
    let existing = dom.semantic_children(node).find(|&c| dom.is(c, ops_w(LocalName::Checked)));
    let mut plan = MutationPlan::new(part);
    touch_field_paragraphs(dom, &mut plan, f);
    match existing {
        Some(c) => plan.node_edits.push(NodeEdit::SetAttr {
            node: Target::Node(c),
            name: ops_w(LocalName::Val),
        ...
        None => {
            ...
            plan.node_edits.push(NodeEdit::Insert {
                parent: Target::Node(node),
                before: None,
                node: NewElement::new(ops_w(LocalName::Checked))
                    .with_attr(ops_w(LocalName::Val), if want { "1" } else { "0" }),
            });
        }
    }
    s.commit_plan(plan)
...
fn set_form_text(
    ...
    if !tracked
        && results.len() == 1
        && let Some(t) = dom.semantic_children(results[0]).find(|&c| dom.is(c, ops_w(LocalName::T)))
    {
        let mut plan = MutationPlan::new(part);
        touch_field_paragraphs(dom, &mut plan, f);
    ...
    touch_field_paragraphs(dom, &mut plan, f);
    let new_run = {
        let mut r = NewElement::new(ops_w(LocalName::R));
        if let Some(p) = &rpr {
            r.push_child(p.clone());
        }
        for seg in text_segments(text, false) {
...
fn set_field_result_props(
    ...
    if let Some(mut t) = Tracker::new(s.document(), ctx) {
        let dom = s.dom();
        let mut plan = MutationPlan::new(part);
        if let Some(p) = dom.ancestors(head).find(|&a| dom.is(a, ops_w(LocalName::P))) {
            plan.touch(p);
        }
        for r in &results {
            if !dom.is(*r, ops_w(LocalName::R)) {
                continue;
            }
...
fn touch_field_paragraphs(dom: &Dom, plan: &mut MutationPlan, f: &crate::span::FieldSpan) {
    let para_of = |n: NodeId| {
        std::iter::once(n).chain(dom.ancestors(n)).find(|&a| dom.is(a, ops_w(LocalName::P)))
    };
    let nodes = f
        .form
        .structure_nodes()
...
pub(super) fn update_block_field(
    ...
    let blocks = materialize_all(s, blocks)?;
    let part = s.main_part();
    let f = field_of(s, id)?;
    if f.lock {
    ...
    if cross {
        ...
        for b in blocks {
            ...
            let nodes: Vec<NewElement> = match b {
                ...
                NewBlock::Xml(e) if e.name == ops_w(LocalName::P) => e
                    .children
                    .into_iter()
                    .filter_map(|c| match c {
                        crate::xml::NewNode::Element(e) if e.name != ops_w(LocalName::PPr) => {
                            Some(e)
                        }
                        _ => None,
                    })
    ...
    loop {
        let grown: Vec<NodeId> = victims
            ...
            .filter(|&p| p != begin_para && p != end_para && !dom.is(p, ops_w(LocalName::P)))
            .filter(|&p| !victims.contains(&p))
            .filter(|&p| ops_live_children(dom, p).all(|c| victims.contains(&c)))
            .collect();
        if grown.is_empty() {
            break;
        }
    ...
    for v in victims {
        match &mut tracker {
            Some(t) if dom.is(v, ops_w(LocalName::P)) || dom.is(v, ops_w(LocalName::Tbl)) => {
    ...
    if ctx.mark_updated_fields_dirty
        && let Some(fld) =
            dom.semantic_children(begin).find(|&c| dom.is(c, ops_w(LocalName::FldChar)))
    {
        // begin run 的 `w:fldChar` 上打 `w:dirty="true"`，Word 打开时重算
        plan.node_edits.push(NodeEdit::SetAttr {
            node: Target::Node(fld),
            name: ops_w(LocalName::Dirty),
            value: "true".into(),
        });
    }
    s.commit_plan(plan)
...
fn revision_ops_w(local: LocalName) -> QName {
    QName::new(NsId::W, local)
}
fn revision_ops_live_children(dom: &Dom, n: NodeId) -> impl Iterator<Item = NodeId> + '_ {
    dom.children(n).iter().copied().filter(|&c| dom.node(c).dirty != Dirty::Deleted)
}
/// 一条修订在某个方向上要做的事。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
fn table_group(s: &EditSession, e: &crate::model::RevisionEntry) -> Vec<RevisionId> {
    ...
    let table_of =
        |n: NodeId| dom.ancestors(n).find(|&a| dom.is(a, revision_ops_w(LocalName::Tbl)));
    ...
    if !accept {
        for job in &jobs {
            ...
            if let Some(grid) = dom.parent(job.node)
                && let Some(table) =
                    dom.ancestors(grid).find(|&n| dom.is(n, revision_ops_w(LocalName::Tbl)))
            {
                let cols: Vec<_> = revision_ops_live_children(dom, grid)
                    .filter(|&n| dom.is(n, revision_ops_w(LocalName::GridCol)))
                    .collect();
...
fn plan_job(
    ...
    match dom.ancestors(job.node).find(|&x| dom.is(x, revision_ops_w(LocalName::P))) {
        Some(p) => plan.touch(p),
    ...
    match act {
        ...
        Act::Drop => {
            ...
            if let Some(parent) = dom.parent(target) {
                keep_cell_paragraph(dom, parent, Some(target), &mut plan);
                // 表格的最后一行也走了 → 整张表跟着走（没有行的 `w:tbl` 不合法）
                if dom.is(target, revision_ops_w(LocalName::Tr))
                    && revision_ops_live_children(dom, parent)
                        .filter(|&c| dom.is(c, revision_ops_w(LocalName::Tr)))
                        .all(|c| c == target)
                {
        ...
        Act::Restore(container, keep) => {
            let c = dom
                .parent(job.node)
                .filter(|&c| dom.is(c, revision_ops_w(container)))
        ...
        Act::Merge => {
            ...
            drop_empty_containers(&mut plan, dom, job.node);
            let para =
                dom.ancestors(job.node)
                    .find(|&x| dom.is(x, revision_ops_w(LocalName::P)))
            ...
            let empty = revision_ops_live_children(dom, para).all(|c| {
            ...
            if empty {
                ...
                if let Some(parent) = dom.parent(para) {
                    keep_cell_paragraph(dom, parent, Some(para), &mut plan);
                }
                return Ok(vec![plan]);
            }
            let merge = plan_merge_with_next(s, part, para)?;
...
fn unwrap(plan: &mut MutationPlan, dom: &Dom, wrapper: NodeId) {
    ...
    plan.structure_changed = true;
    for c in revision_ops_live_children(dom, wrapper).collect::<Vec<_>>() {
        plan.node_edits.push(NodeEdit::Move {
            node: c,
            parent: Target::Node(parent),
            before: Some(wrapper),
...
fn drop_empty_containers(plan: &mut MutationPlan, dom: &Dom, marker: NodeId) {
    ...
    while let Some(c) = cur {
        ...
        if revision_ops_live_children(dom, c).any(|x| !gone.contains(&x)) {
            break;
        }
        plan.node_edits.push(NodeEdit::Delete(c));
        gone.push(c);
...
fn restore(
    ...
    let mut kept = 0usize;
    for c in revision_ops_live_children(dom, container).collect::<Vec<_>>() {
        if c == change {
            continue;
        }
        let Some(name) = dom.name(c) else { continue };
    ...
    let snapshot =
        revision_ops_live_children(dom, change).find(|&c| dom.is(c, revision_ops_w(inner)));
    ...
    if let Some(snapshot) = snapshot {
        ...
        let order = crate::semantic::props::TABLES
            ...
            .map(|t| t.order_index);
        for c in revision_ops_live_children(dom, snapshot).collect::<Vec<_>>() {
            let before = order
                .zip(dom.name(c).and_then(|q| order.and_then(|f| f(q))))
                .and_then(|(f, mine)| {
                    revision_ops_live_children(dom, container)
                        .filter(|&k| k != change)
...
fn plan_drop_cells(s: &mut EditSession, job: &Job) -> Result<Vec<MutationPlan>> {
    ...
    let cell = dom
        ...
        .filter(|&c| dom.is(c, revision_ops_w(LocalName::Tc)))
    ...
    let table = dom
        .ancestors(cell)
        .find(|&a| dom.is(a, revision_ops_w(LocalName::Tbl)))
    ...
    let dom = s.dom_in(part)?;
    let marked = |c: NodeId| {
        revision_ops_live_children(dom, c)
            .find(|&x| dom.is(x, revision_ops_w(LocalName::TcPr)))
            .is_some_and(|tcpr| {
                revision_ops_live_children(dom, tcpr).any(|m| dom.is(m, revision_ops_w(mark)))
            })
    };
...
fn sdt_ops_w(local: LocalName) -> QName {
    QName::new(NsId::W, local)
}
...
fn guard(s: &EditSession, sdt: NodeId) -> Result<NodeId> {
    ...
    if (sdt.0 as usize) >= dom.node_count()
        ...
        || !dom.is(sdt, sdt_ops_w(LocalName::Sdt))
    {
    ...
    match info.refusal() {
        ...
        None => dom
            ...
            .find(|&c| dom.is(c, sdt_ops_w(LocalName::SdtContent)))
...
pub(crate) fn set_sdt_content(
    ...
    if dom
        ...
        .any(|c| dom.is(c, sdt_ops_w(LocalName::P)) || dom.is(c, sdt_ops_w(LocalName::Tbl)))
    {
        return Err(unsupported("这个内容控件装的是块级内容；请对里面的段落用 ReplaceInlines"));
    }
    let para = dom
        .ancestors(content)
        .find(|&a| dom.is(a, sdt_ops_w(LocalName::P)))
        .ok_or_else(|| Error::edit(DiagCode::EditBadPosition, "内联控件不在段落里"))?;
    replace_container_inlines(s, para, content, inlines, ctx)
}
...
pub(crate) fn remove_sdt_shell(s: &mut EditSession, sdt: NodeId) -> Result<MutationResult> {
    ...
    let mut plan = MutationPlan::new(s.main_part());
    plan.structure_changed = true;
    if let Some(p) = dom.ancestors(sdt).find(|&a| dom.is(a, sdt_ops_w(LocalName::P))) {
        plan.touch(p);
    }
...
fn section_ops_w(local: LocalName) -> QName {
    QName::w(local)
}
...
fn require_sect_pr(dom: &Dom, sect: NodeId) -> Result<()> {
    let live = (sect.0 as usize) < dom.node_count()
        && dom.is(sect, section_ops_w(LocalName::SectPr))
...
fn reference_of(dom: &Dom, sect: NodeId, kind: HfKind, variant: HfVariant) -> Option<NodeId> {
    ...
    dom.semantic_children(sect)
        .filter(|&n| dom.is(n, section_ops_w(elem)) && dom.node(n).dirty != Dirty::Deleted)
        .find(|&n| {
            // `w:type` 缺失与非 schema 的 `odd` 都算 default（`RES-10`）
            let kind = dom.attr_value(n, section_ops_w(LocalName::Type));
...
fn reference_site(dom: &Dom, sect: NodeId) -> Option<NodeId> {
    dom.semantic_children(sect).filter(|&n| dom.node(n).dirty != Dirty::Deleted).find(|&n| {
        !dom.is(n, section_ops_w(LocalName::HeaderReference))
            && !dom.is(n, section_ops_w(LocalName::FooterReference))
    })
}
/// 新引用元素。
fn reference_element(kind: HfKind, variant: HfVariant, rid: &str) -> NewElement {
    ...
    let mut e = NewElement::new(section_ops_w(elem));
    e.push_attr(section_ops_w(LocalName::Type), variant.as_str().to_string());
...
fn replace_part_blocks(
    ...
    let content = materialize_all(s, content)?;
    let mut tracker = Tracker::new(s.document(), ctx);
    let dom = s.dom_in(Some(part))?;
    let root = dom.root();
    let mut plan = MutationPlan::new(part);
    ...
    for c in dom.children(root).iter().copied() {
        ...
        match &mut tracker {
            ...
            Some(t) => plan_delete_block_tracked(&mut plan, dom, t, c),
...
pub(super) fn set_page_color(s: &mut EditSession, color: Option<String>) -> Result<MutationResult> {
    ...
    let root = dom.root();
    let existing = dom.semantic_children(root).find(|&n| {
        dom.is(n, section_ops_w(LocalName::Background)) && dom.node(n).dirty != Dirty::Deleted
    });
    let mut plan = MutationPlan::new(s.main_part());
    match (color, existing) {
        ...
        (Some(c), Some(n)) => plan.node_edits.push(NodeEdit::SetAttr {
            node: Target::Node(n),
            name: section_ops_w(LocalName::Color),
            value: c,
        }),
        (Some(c), None) => {
            let mut e = NewElement::new(section_ops_w(LocalName::Background));
            e.push_attr(section_ops_w(LocalName::Color), c);
            plan.node_edits.push(NodeEdit::Insert {
                parent: Target::Node(root),
...
pub(super) fn set_watermark(
    ...
    let (root, doomed, before) = {
        ...
        let doomed: Vec<NodeId> = dom
            ...
            .copied()
            .filter(|&c| {
                dom.node(c).dirty != Dirty::Deleted && dom.is(c, section_ops_w(LocalName::P))
            })
            .filter(|&c| {
...
pub(super) fn insert_section_break(
    ...
    if (after.0 as usize) >= dom.node_count()
        ...
        || !dom.is(after, section_ops_w(LocalName::P))
    {
    ...
    if !dom.is(parent, section_ops_w(LocalName::Body))
        && !dom.is(parent, section_ops_w(LocalName::SdtContent))
    {
        return Err(Error::edit(
            DiagCode::EditBadPosition,
    ...
    let target = match ppr {
        Some(p) => {
            let before = dom
                ...
                .find(|&c| dom.is(c, section_ops_w(LocalName::PPrChange)));
        ...
        None => {
            ...
            plan.node_edits.push(NodeEdit::Insert {
                ...
                before: first,
                node: NewElement::new(section_ops_w(LocalName::PPr)),
            });
            plan.node_edits.push(NodeEdit::InsertClone {
                parent: Target::New(k),
                before: None,
...
pub(super) fn delete_section_break(
    ...
    let ppr = dom
        .parent(sect)
        .filter(|&p| dom.is(p, section_ops_w(LocalName::PPr)))
    ...
    if let Some(para) = dom.parent(ppr).filter(|&x| dom.is(x, section_ops_w(LocalName::P)))
...
impl EditSession {
    ...
    pub fn apply(&mut self, op: EditOp, ctx: &EditContext) -> Result<MutationResult> {
        self.transaction(|s| run(s, op, ctx))
    }
...
pub(crate) const EMU_PER_PT: f64 = 12700.0;
...
fn anchor_open(wp: &str, cx: i64, cy: i64, look: &ShapeLook, id: i64, name: &str) -> String {
    let wrap = look.wrap.unwrap_or(media_ops::ImageWrap::SquareLeft);
    let behind = if wrap == media_ops::ImageWrap::Behind { "1" } else { "0" };
    ...
    let wrap_el = match wrap {
        media_ops::ImageWrap::TopBottom => format!("<{wp}:wrapTopAndBottom/>"),
        media_ops::ImageWrap::Front | media_ops::ImageWrap::Behind => format!("<{wp}:wrapNone/>"),
...
pub(crate) fn shape_paragraph(s: &mut EditSession, block: NewBlock) -> Result<NewElement> {
    ...
    let inner = materialize_all(s, inner)?;
    let dom = s.package_mut().dom_mut(main)?.expect("main part parsed");
    let inner: Vec<NewElement> = inner.into_iter().map(|b| new_block_element(dom, b)).collect();
    ...
    let xml = match &block {
        ...
        NewBlock::Line { kind, from, to, color } => {
            ...
            let look = ShapeLook {
                ...
                wrap: Some(media_ops::ImageWrap::Front),
                ..Default::default()
            };
...
pub(crate) fn track_site_of(dom: &Dom, node: NodeId, stop: NodeId, author: &str) -> TrackSite {
    let mut x = Some(node);
    while let Some(n) = x {
        if n == stop {
            break;
...
impl Tracker {
    ...
    pub(crate) fn para_mark(
        ...
        let ppr = ppr_of(dom, para);
        let rpr = ppr.and_then(|p| track_child_named(dom, p, track_w(LocalName::RPr)));
        // 已经有同种标记就不重复加
        if let Some(r) = rpr
            && track_live_children(dom, r).any(|c| dom.is(c, track_w(local)))
        {
            return;
        }
        ...
        match (ppr, rpr) {
            (_, Some(r)) => {
                ...
                let before = track_live_children(dom, r).find(|&c| {
                    dom.name(c)
            ...
            (Some(p), None) => {
                ...
                let before = track_live_children(dom, p).find(|&c| {
            ...
            (None, None) => {
                ...
                let before = track_live_children(dom, para).next();
                plan.node_edits.push(NodeEdit::Insert {
                    parent: Target::Node(para),
                    before,
    ...
    pub(crate) fn container_mark(
        ...
        let existing = track_child_named(dom, owner, track_w(container));
        if let Some(c) = existing
            && track_live_children(dom, c).any(|x| dom.is(x, track_w(mark)))
        {
            return;
        }
        ...
        match existing {
            Some(c) => {
                ...
                let before = track_live_children(dom, c)
    ...
    pub(crate) fn snapshot(
        ...
        if track_live_children(dom, container).any(|c| dom.is(c, track_w(change))) {
            return None;
        }
        let k = plan.node_edits.len();
        plan.node_edits.push(NodeEdit::Insert {
        ...
        for c in track_live_children(dom, container).collect::<Vec<_>>() {
            let Some(name) = dom.name(c) else { continue };
...
fn track_live_children(dom: &Dom, n: NodeId) -> impl Iterator<Item = NodeId> + '_ {
    dom.children(n).iter().copied().filter(|&c| dom.node(c).dirty != Dirty::Deleted)
}
fn track_child_named(dom: &Dom, parent: NodeId, name: QName) -> Option<NodeId> {
    track_live_children(dom, parent).find(|&c| dom.is(c, name))
}
...
mod test_edit {
    ...
    mod mod_test_edit {
        ...
        fn targets_preserve_parts_duplicates_order_and_snapshot_ownership() {
            let session = EditSession::blank(None).unwrap();
            let header = InlinePos::in_part(PartId(7), NodeId(11), 0);
            let op = EditOp::DeleteRange { from: header, to: header.with_offset(1) };
        ...
        fn position_utf16_text_conversion_preserves_boundaries_and_errors() {
            ...
            for (units, expected_code, expected_message) in [
                (2, DiagCode::EditSplitSurrogate, "偏移落在代理对中间"),
                (5, DiagCode::EditBadPosition, "偏移超出段文本"),
            ] {
                let error =
                    usize::try_from(Utf16TextOffset { text: &text, offset: Utf16Offset(units) })
                        .unwrap_err();
        ...
        fn position_json_protocol_is_unchanged() {
            let position = InlinePos::in_part(PartId(7), NodeId(11), 3);
            ...
            let main = InlinePos::new(NodeId(11), 0);
            assert_eq!(
    ...
    mod session_relative_target_tests {
        use super::*;
        use relative_target;
        #[test]
...
// ===== END crates/rsword/src/edit/mod.rs =====

// ===== BEGIN crates/rsword/src/edit/atom_ops.rs =====
//! `EDIT-03 InsertAtom`：往坐标流里插一个**原子**（`spec/18` 7.5）。
//!
//! 原子在坐标流里恒占 **1 个 UTF-16 单位**（`EDIT-02`），所以插入前后的偏移差正好是 1。
//! 五种原子共用 [`super::ops`] 的边界定位（拆 run → `boundary_site`）与追踪时的
...
// ===== END crates/rsword/src/edit/atom_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/chart_ops.rs =====
//! 图表的写侧（`EDIT-03` / `SAVE-05` / `SAVE-06`，`spec/17` 任务 6.6）：改缓存文本（`set_chart_data`）、
//! 新建图表（`materialize`：图表 part + 内嵌工作簿 + 关系 + 绘图段落）。
//!
//! `SetChartData` 只改缓存文本节点（TS `patchChartPartXml` 的语义）：数据引用 `c:f`、样式、布局一个字节不动，
...
// ===== END crates/rsword/src/edit/chart_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/drawing_ops.rs =====
//! 既有绘图的编辑（`EDIT-03`，`spec/18` 7.7）：尺寸 / 旋转 / 翻转 / 裁剪、z-order、形状样式。
//!
//! **只改属性**（分层决策 9）：`wp:extent` / `a:ext` / `wp:posOffset` / `relativeHeight` /
//! `a:xfrm` 的属性改动让那个元素 `SelfDirty`，`a:graphic` 子树永远原字节。
...
// ===== END crates/rsword/src/edit/drawing_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/field_ops.rs =====
//! 块字段的收集与重算（`FLD-09`，`spec/18` 7.8）。
//!
//! 生成器（`span::field::generate`）只把算好的条目摊成 XML；从文档里**收**条目在这里——
//! 走标题、收 `XE` 词、数 `SEQ`，都要 `EditSession`。
...
// ===== END crates/rsword/src/edit/field_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/ink_ops.rs =====
//! 墨迹的写侧（`SAVE-07 inks`、`EDIT-03` / `EDIT-06`，`spec/17` 任务 6.8）。
//!
//! `inks` 是**权威列表**（与 `comments` 同语义）：`EditSession::remove_inks` 删掉主 part 里全部墨迹 run
//! （它们的媒体与关系随保存时的资源回收消失，6.7），再对每条 [`InkSave`] 调 `EditSession::insert_ink`
...
// ===== END crates/rsword/src/edit/ink_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/inline.rs =====
//! 新内容描述（`docs/03` §8.2 的 `NewInline`）与其 `New` 子树生成。
//!
//! 文本里的控制字符折回段种类（`COMPAT-08`）：`\t` → `w:tab`，`\n` → `w:br`，`\u{0C}` → `w:br w:type="page"`，
//! `\u{0B}` → `w:br w:type="column"`，`\r` → `w:cr`；其余进入 `w:t`（`New` 节点序列化时一律带
...
// ===== END crates/rsword/src/edit/inline.rs =====

// ===== BEGIN crates/rsword/src/edit/media_ops.rs =====
//! 媒体写侧（`PKG-05` / `EDIT-04` / `EDIT-06` / `SAVE-05`，`spec/17` 任务 6.7）：新图片段落、`ReplaceImageMedia`、
//! 按内容去重的媒体 part。资源回收在 `save/prune.rs`。
//!
//! 新图片的段落按 TS `embedImage` / `applyImageWrap` 的模板生成（`wp:inline`，或带 `wrap` 时 `wp:anchor`），
...
// ===== END crates/rsword/src/edit/media_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/note_ops.rs =====
//! 注释条目的内容操作（`EDIT-03`，`spec/18` 7.5）：`SetNoteContent` / `RemoveNote`。
//!
//! 条目内容本身在注释 part 的内容流里，改文字用 `InlinePos { part: 注释 part }` 上的
//! `InsertText` / `DeleteRange` 就行（TS 的 `text-patch` 场景就是这么做的，格式与超链接
...
// ===== END crates/rsword/src/edit/note_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/ops.rs =====
//! `EDIT-03` 操作实现（M1 子集）。每个操作是一个或多个 plan/commit 阶段；事务边界在
//! [`EditSession::apply`]（失败整体回滚）。这里的函数只读 DOM 与投影、产出 [`MutationPlan`]，
//! 写入全部经 `EditSession::commit_plan`。
...
use super::{BlockAt, BlockPos, EditContext, EditOp, EditSession, NewBlock, NewInline, NewRun};
use super::{InlineDiff, InlinePos, Loc, Step, Tok, Utf16Offset, Utf16TextOffset};
...
pub(crate) fn run(s: &mut EditSession, op: EditOp, ctx: &EditContext) -> Result<MutationResult> {
    ...
    // 7.7：位置落在 `mc:Fallback` 里直接拒；落在 `mc:Choice` 的文本框里的记下来，提交后同步孪生
    let targets = s.op_targets(&op);
    let twins = super::twin::sites(s, targets.clone())?;
    // 几何与样式不改内容，改的是 VML 那边的 `@style` / `@fillcolor` / `@strokecolor`
    let styled = matches!(op, EditOp::SetDrawingGeometry { .. } | EditOp::SetShapeStyle { .. })
        .then_some(targets);
    // `spec/18` 7.3：Word 自己也不把这些记成修订（或另有机制）。照常执行，留一条
...
fn replace_inlines_tracked(
    ...
    for i in inlines {
        ...
        em.emit(i, false, &mut out);
        match i {
            NewInline::Run(r) => new_toks.push(Tok::Run(&r.text, r.props.clone().map(Box::new))),
            other => {
                has_marker |= matches!(other, NewInline::Marker(_));
                new_toks.push(Tok::Atom(format!("{other:?}")));
            }
...
// ===== END crates/rsword/src/edit/ops.rs =====

// ===== BEGIN crates/rsword/src/edit/plan.rs =====
//! `EDIT-05`：`MutationPlan`（只读产出）→ `validate`（只读）→ `commit`（机械写入）→ `MutationResult`。
...
// ===== END crates/rsword/src/edit/plan.rs =====

// ===== BEGIN crates/rsword/src/edit/revision_ops.rs =====
//! 接受 / 拒绝修订（`EDIT-03` 的那张表，`spec/18` 7.4）。
//!
//! **接受 / 拒绝是普通 `EditOp`**（分层决策 4）：走 plan / validate / commit，`AcceptAll` 是**一个
//! 事务**（任一步失败整体回滚，`EDIT-05`），事务内部按修订逐条提交——顺序是文档序、
...
// ===== END crates/rsword/src/edit/revision_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/sdt_ops.rs =====
//! 内容控件的内容操作（`EDIT-03`，`spec/18` 7.5）：`SetSdtContent` / `RemoveSdtShell`。
...
// ===== END crates/rsword/src/edit/sdt_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/section_ops.rs =====
//! 节与页眉页脚的编辑操作（`EDIT-03`、`SAVE-05`、`SAVE-07`，`spec/16` 任务 5.5）。
//!
//! 五个操作，都走 `plan → validate → commit` 的同一条路（`EDIT-05` 的事务边界在
//! [`EditSession::apply`]），没有旁路：
...
// ===== END crates/rsword/src/edit/section_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/session.rs =====
//! `EDIT-01` 会话：`Package`（规范状态）+ `Document`（投影）+ 事务（`EDIT-05`）。
...
// ===== END crates/rsword/src/edit/session.rs =====

// ===== BEGIN crates/rsword/src/edit/shape_gen.rs =====
//! 新建浮动文本框 / 形状 / 线条（`spec/18` 7.7 的 `NewBlock::Textbox / Shape / Line`）。
//!
//! 都是 DrawingML 的 `wps:wsp`。Transitional 包按 Word / TS 的形态发一对
//! `mc:Choice Requires="wps"` + `mc:Fallback`（VML 孪生）；**Strict 包只发 Choice**——
...
// ===== END crates/rsword/src/edit/shape_gen.rs =====

// ===== BEGIN crates/rsword/src/edit/table_ops.rs =====
//! 表格的行列结构操作（`EDIT-03` 表格段，任务 3.8）。
//!
//! **几何以声明网格为准**：列数 = `tblGrid/gridCol` 个数，行宽 = `gridBefore + Σ gridSpan + gridAfter`。
//! 任一行的宽度与列数不符时，列操作与 `MergeCells` 直接 `Err(EDIT_TABLE_GRID_INCONSISTENT)`——**不**
...
// ===== END crates/rsword/src/edit/table_ops.rs =====

// ===== BEGIN crates/rsword/src/edit/tests.rs =====
use std::io::{Cursor, Write};
...
// ===== END crates/rsword/src/edit/tests.rs =====

// ===== BEGIN crates/rsword/src/edit/track.rs =====
//! 修订生成（`EDIT-03` 每个操作的「修订」行；`spec/18` 7.2 / 7.3）。
//!
//! **修订是计划的一部分，不是事后包装**（`spec/18` 分层决策 1）：包裹、改名、快照都是普通
//! [`NodeEdit`]，由 [`Tracker`] 在 plan 阶段生成，`MutationPlan` 不加字段。`w:id` 从会话里全包
...
// ===== END crates/rsword/src/edit/track.rs =====

// ===== BEGIN crates/rsword/src/edit/twin.rs =====
//! `mc:AlternateContent` 的孪生同步（`spec/18` 7.7）。
//!
//! Word 写文本框与形状时发两份：`mc:Choice Requires="wps"` 里的 DrawingML（新版读这份）与
//! `mc:Fallback` 里的 VML（老版读那份）。两份是同一个对象的两种写法，内容必须一致。
...
// ===== END crates/rsword/src/edit/twin.rs =====

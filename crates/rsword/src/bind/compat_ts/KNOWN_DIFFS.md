...
| `vml-textbox__007` | `blocks[*].textboxes[*].paras[*].runs[*].instrField` | 框里 `DATE` 字段的 `instrField` | 字段折叠整体在 M2（顶层 `blocks[*].runs[*].instrField` 同样未实现），跟绘图无关 | 暂放行，M2 接字段后删除 |
...
vml-textbox__007*        *paras[*].runs[*].instrField             # 字段折叠在 M2

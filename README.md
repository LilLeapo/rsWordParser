...
架构 v3.2 已冻结（2026-09-03）。**M0、M1、M2 均已完成并并入 `main`**（M0/M1 2026-09-04，M2 2026-09-05）：
字节保真的读写骨架、属性表、文本段落模型、`resolve` 首版、`compat_ts`（文本 + 字段 + 批注）、
编辑引擎（`EditSession` + 内联 / 块 / 段落 / 范围 / 字段操作 + 事务）、L2 范围层（`Anchor` 变换与物化）、
字段子系统（配对 / 指令 tokenizer / 策略表）、批注与注释（含 `SAVE-05` 新建 part）、`SaveBlock[]` 与保存
选项的兼容映射、差分工具链与 fuzz 目标。M1 门三条、M2 门两条都跑过（CI 里各一步）。

能力矩阵、实测数字、明确未实现的清单在 **`docs/05-status.md`**；任务清单与偏差记录在
`docs/04-dev-plan.md`（§11 是 M2 的逐条进度）。M4（绘图）有并行分支在做；M3（表格）尚未开工，
任务分解待写。

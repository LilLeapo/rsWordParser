...
编译时从**正向线型 EditOpJson**生成可无损还原的审计记录，不依赖 BIND-03 可能具名拒绝的引擎→JSON 反向出口。
...
执行线型仍含原生必填 bytes；审计记录将该字段外置，通过操作序号、字段路径、SHA-256、长度、MIME 五要素绑定附件。
审计记录本身不冒充合法 EditOpJson；重放先验证附件并还原字节，再按原生类型解析。附件缺失返回 AGENT_ATTACHMENT_MISSING；
内容、SHA-256、长度或 MIME 不匹配返回 AGENT_ATTACHMENT_MISMATCH，禁止替代字节、跳过操作或部分重放。
...
每项 W 操作审计序列在验证附件并还原后可解析，规范序列与实际执行线型序列逐字节相等；重放到相同初始快照（媒体输入一并提供）得到相同语义结果。
正向断言还原等式；反向分别篡改附件内容、摘要与 MIME，或删除附件，必须具名失败，不能只测无篡改往返。

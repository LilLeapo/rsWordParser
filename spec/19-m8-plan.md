...
5. **既有门不退**：`--features compat-ts` 下，九道 `diff-parse` 门（七个 scope + `corpus/real` + `save_blocks`）继续 0 处未知差异；
   **新增（2026-09-09）**：一道**全语料「无编辑 `EditSession::save()` 字节相同」门**（不变式 1 的包级形态）。
   补它的起因：`spec/21` v3 ① 那处冲突之所以从 M0 潜伏到 8.3 才暴露，是因为 `tests/xml_roundtrip.rs`
   只测每个 XML part 的 `parse → serialize`（L1 序列化器），全仓的「无编辑保存字节相同」断言只散落在
   几份**手挑**文档上（`cell_edit.rs` / `field.rs` / `hf_ops.rs` / `blank.rs`），**没有任何门看这条不变式的
   包级形态**。落在 8.6，与自快照网一起进 CI；已知的例外（若有）必须点名成清单、双向锁死。
...
| 8.3 | **`EditOp` / `EditContext` / `MutationResult` 的 JSON**（`edit/mod.rs` 的 serde、`build/props.rs` 给 patch 类型加 serde）：**（2026-09-09 订正：本行原写「`EditOp` 与 `NewBlock / …` 全部 `Serialize / Deserialize`」，那是 `spec/21` BIND-03 **v1** 的措辞，已随 v2「线型与引擎型分离」作废——v2 时只订正了下文「实现约定」一节，漏了本行。照 `spec/21` BIND-03 v2/v3 执行：线型 `EditOpJson` derive serde，引擎 `EditOp` **禁止** derive，转换带 `&Dom` 上下文。）**属性表 `*Patch` 的 serde 由 `build/props.rs` 生成——`PROP-06` 的 diff / patch 形态已有）。`edit_op_json!` 宏展开每个变体的往返测试与 `spec/21` `BIND-03` 的清单（生成后人工校对），并在 `InsertBlock{Xml}` / `ReplacePartXml` 两处打开 `BIND_XML_ESCAPE` 计数点。`SaveOptions` 里只为 TS 存在的翻译入口（5.6 / 5.7 / 6.6–6.8 的节 / 页眉页脚 / 水印 / 编号 / 样式 / 主题 / 墨迹）**公开为 `EditOp`**，`SaveOptions` 收缩到 `BIND-04` 的五项 | EDIT-01–EDIT-06, PROP-06, BIND-03, BIND-04 | 门 2 全绿；`SaveOptions` 只剩五个包级键，其余有对应 `EditOp` 且各有一处测试 |

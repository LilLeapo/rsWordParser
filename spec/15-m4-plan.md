...
**进度**：4.1 完成（`package/media.rs` + `tests/media.rs` 语料普查；同时给 L1 补了 `Dom::semantic_descendants`）。4.2 完成（`resolve/drawingml.rs` 颜色 + `model/units.rs` 单位换算；语料 89 个颜色容器全部能定出 sRGB）。4.3 完成（`model/drawing.rs` 挂到 `Segment.display`；112 个绘图、122 项 `wp:extent` 与 TS 一致）。

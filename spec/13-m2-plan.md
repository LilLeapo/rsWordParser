...
| # | 任务 | 规范 | DoD |
| --- | --- | --- | --- |
| 2.1 | Span 索引：`RangeSpan` / `RangeKind` / `SpanId`，按 `FlowId` 构建；起终点配对、同流、起在终前；孤儿终点删除、反序交换 | SPAN-01–SPAN-05, SPAN-09 | 全语料标记成对；`corpus/hostile/span-orphan-end.docx` 记 `SPAN_ORPHAN_END` 且保存成功 |
| 2.2 | `Anchor` / `Affinity`、`dom.compare` 文档序、四条变换规则（插入 / 删除 / 拆分 / 合并） | SPAN-06, SPAN-07 | `EDIT-03` 验收行：删除覆盖书签起点后起点落到删除处；`DeleteRange` 不再记 `EDIT_ANCHOR_UNMOVED` |
| 2.3 | Span 物化与保存校验：接进 `EditSession::save_with` 的第 3 步 | SPAN-08, SAVE-02 | 编辑后标记位置与 Anchor 一致；注入破坏的 Span 在调试构建下 `Err(SAVE_INVARIANT)` |
...
5. **语料基线**：M2 开工前应在干净的 genoffice 基线上重导语料（`docs/04` §9 第 1 条），否则 2.5 / 2.6 的
   差分基准会随 TS 工作树漂移。

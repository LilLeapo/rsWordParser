...
| 2.8 ✅ | `EDIT-06` id 分配落地（`rId` / 批注 / 书签 / `paraId` 都在，修订 `w:id` 随 2.6 / 2.9）：`rId`（新外链）、书签 `w:id`、批注 `w:id`、修订 `w:id` 预留、`w14:paraId` | EDIT-06 | 验收清单 EDIT-06；`insert-and-layout__001.save.10`（新超链接关系）通过 |
...
## 从 M1 带过来的债（M2 内解决）—— 全部已清

| 债 | 位置 | 解决任务 | 结果 |
| --- | --- | --- | --- |
| `DeleteRange` 覆盖范围标记时"标记不动"并记 `EngineInvariantViolation` | `edit/ops.rs` | 2.2 ✅ | 锚点按 `SPAN-06` 变换；标记删掉内容项后物理上就落在删除点，不必重写，诊断撤掉 |
| `DeleteRange` 覆盖字段结构段时整 run 保留，只删文本段 | `edit/ops.rs` | 2.4 ✅ | 对**已识别**字段这就是正确行为（透明字段的结果可编辑）；只有畸形字段的 `fldChar` 才记诊断 |
| `save_with` 第 3 步（Span 物化）是空操作 | `edit/session.rs` | 2.3 ✅ | `SPAN-08` 物化 + `SPAN-09` 校验接进去了 |
| 缺 `word/settings.xml` 时清洗标志写不进去，只记诊断 | `save/options.rs` | 2.6 ✅ | `SAVE-05` 按需新建 part（只在要写 `true` 时） |
| 新外部超链接无法分配 `rId`，`apply_save_blocks` 直接 `EditUnsupported` | `bind/compat_ts/save_blocks.rs` | 2.8 ✅ | `EDIT-06` 分配 `rId`；part 没有 `.rels` 时也会建 |

...
逐条进度见 `docs/04` §13。M4 是从 M2 之前的 `main` 分出来的，所以走的是真合并；整合时的三处
调整记在 §13 末尾。下一个里程碑是 **M3（表格）**，分支 `m3-tables`。
...
**不能**：表格的模型与操作（M3）、绘图的**编辑**与写回（M7，读侧已完成）、页眉页脚 / 节 / 水印 /
图表等保存选项（M5 / M6）、块字段生成器与修订生成（M7）。
...
| L3 模型 `model/` | 文本 + 字段 + 批注 / 注释 + 绘图 | `Document::rebuild`、块分类 R01–R19（含 R09 字段块）、段落坐标流（`Run`/`Segment`，UTF-16）、`Inline::Field` 与透明字段、`ParagraphFacts`、声明模型（styles / numbering / theme / settings / fontTable / comments / footnotes / endnotes）、绘图 / 形状 / VML 显示模型与节页面几何（`Segment.display` / `ProtectedBlock.display` / `ImageBlock.display`） | 表格模型（M3） |
| resolve `resolve/` | 首版 | 样式链（basedOn / link）、docDefaults 层叠、每字段 `Provenance`、主题字体与颜色、符号字体解码、heading 级别、DrawingML 颜色算法 | toggle 属性真实规则 + Word 实测 fixture（M5）、补全 Wingdings 2/3 与 Webdings 映射表 |
| L4 编辑 `edit/` | 段落 / 范围 / 字段操作齐了 | `EditSession`（含范围索引）、`InlinePos` 定位、`MutationPlan` plan/validate/commit、按 part 回滚的事务（DOM + 索引）、`InsertText`、`DeleteRange`（同段）、`SetRunProps`、`SetParaProps`、`ReplaceInlines`、`ReplaceParaProps`、`InsertBlock`/`DeleteBlock`/`MoveBlock`、`SPAN-06/07` 锚点维护、`AddComment`/`RemoveComment`/`SetCommentText`（含 `SAVE-05` 新建 part）、`SplitParagraph`/`MergeWithNext`、`AddBookmark`/`RemoveBookmark`、`InsertField`/`SetLinkTarget`/`ToggleCheckbox`/`SetFormText`/`SetFieldResultProps`/`UpdateBlockField` | 表格操作（M3）、块字段生成器与修订生成（M7） |
...
| 兼容 `bind/compat_ts/` | 文本 + 字段 + 批注 / 注释 + 绘图 | `parsed_doc` 整份 `ParsedDoc`（含 `extras`、UTF-16 索引、sdt 拆分）、字段折叠 run 与 `fieldDisplay` / `fieldLabel`、`comments` / `footnotes` / `endnotes` / `commentIds` / `noteRef`、整个绘图域（`image*` / `textboxes[]` / `rule*` / `oleProgId`）、`apply_save_blocks`（original / generated / xml 块）、容忍差分 | 表格 / 页眉页脚字段（随对应里程碑） |
...
| 指标 | 值 | 来源 |
| --- | --- | --- |
| 源码行数 / 文件数 | 40,971 行 / 107 个（另有生成代码 16,793 行） | `find crates tools -name '*.rs' \| xargs wc -l` |
| 测试数 | 297（单元 + 集成，20 个集成测试文件） | `cargo test --workspace` |
...
| 解析差分（绘图域，M4 门） | 573 份用例，**0 处未知差异**（按**路径**筛，不是按文档） | `cargo run -p diff-parse -- --scope drawing` |
| 解析差分（全域） | 573 份里 138 份有未知差异、295 个差异点（M3 / M5 / M6 的工作面） | `cargo run -p diff-parse -- --scope all` |
| 媒体解析 | 589 份文档 106 处 `a:blip` / `v:imagedata` 引用：包内 93（89 位图 + 4 metafile）、外链 4、文档本身就坏 9 | `cargo test -p rsword --test media -- --nocapture` |
| 绘图事实 | 125 个 `w:drawing`（98 形状 / 17 图片 / 9 组），锚定 114；138 项 `wp:extent` 与 TS 的 `imageWidthPx/HeightPx` 一致 | `cargo test -p rsword --test drawing -- --nocapture` |
| VML 与嵌入对象 | 43 个 `w:pict`/`w:object`、61 个形状；细横线 1、带图 16、带文本框 20、嵌入对象 13 | 同上 |
...
全域差异按域聚合（差异点，M2 + M4 之后实测 295）：页眉页脚 141（M5：`hfParts` 46 +
`header*`/`footer*` 95）、表格 67（M3）、块分类连带项与 run 约 40（多半是表格与页眉页脚的连带）、
图表与公式约 10（M6），其余零散。文本域、字段与 Span 域、绘图域三道门都是 0。
...
- **新建 part**：`SAVE-05` 已落地（批注 / `commentsExtended` / 脚注 / 尾注 / `settings.xml` / 缺失的
  ...
  图表 part（随 M5 / M6）。
- **表格**：块层面是占位（`Table`），单元格属性不进模型（M3）。
- **绘图**：读侧完整（显示模型 + 投影 + 门），**编辑与写回没有**——`applyImageZOrder` 的
  `relativeHeight` 回写、`xml.replaceImage` 都在 M7。另外三块按分层留给后面：表格单元格里的图片
  （M3）、页眉页脚里的图片（M5）、图表 / SmartArt / lockedCanvas / 墨迹的**内容**（M6，M4 只做了
  `w:object` 的 ProgID 与预览图）、外部文本框 part（`wps:txbx r:txbx`，要 M5 的跨 part 内容流）。
...
cargo test --workspace && cargo test --workspace --release          # 297 个测试，两种构建

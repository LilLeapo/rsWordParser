...
find 增加 pageHits/hasMore；首句工具说明明确本页不是总数，字段在预算选前缀之前计费，CLI/MCP 共用 schema。
常规进程测试锁住 main 两页共 3 处 / all 八页共 15 处 / 空命中 0，以及每页计数、游标与实际字节数。
实际把 pageHits 改为 end+1 后断言报红（3 对 2），逐字节恢复后重跑。
docs/17 的全部 action 完整请求直接被运行器与测试读取执行，10 个执行成功、updateToc 按清单具名拒绝；
另执行文档的 find 源锚点 → paragraph ObjectRef → addComment 流程，重开恰有一条批注。
这些新增测试不是额外的真实 Agent 任务通过数。核心引擎、语料与原生操作语义未改；寻址风格并排说明，未扩大 scope 契约。

9.9 串行实跑默认 debug/release 各 **955 / 0 / 13**、compat 各 **1074 / 0 / 13**；各增加三条常规测试。
fmt 干净，两套 clippy/audit/doc 零告警；八道差分 **242 + 547 已知 / 0 未知**。
save_blocks **204/208 等价、4 份既有具名差异、0 跳过**，体积 **251 份、−67.5%**；1099 快照未改、无流段落门精确 0。
debug/release 的 CLI/MCP 进程探针通过；这是传输回归，不是本轮真实 MCP Agent 任务。
未执行可选 JS 字节对照、Linux 运行或定时 fuzz 长跑，不冒充本轮新证据。日志在 target/m99-final；计数破坏验证在 target/m99/sabotage.log。
下面 9.8 的基准保留当轮口径；新增 find 信封字段后，不把旧表的 find 字节数当作当前响应字节数。

...
> **已完成**（2026-09-07 交付并接入，见 `docs/04` §15 与 `corpus/real/_round3/`）。四件事全部有结论：
> 任务 A 的对照件进了 `fixtures/{revisions,word-ops}`；任务 B 复验通过（9/9 + 4/4）；任务 C 确认兼容模式与 toggle 规则无关；
> 任务 D 两项未达成，但独立复核后确认是 Word / 绘图工具的限制。**真实 Word 这条线到此收尾。**

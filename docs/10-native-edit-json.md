...
`edit_op_json!` 的同一张表展开 67 个线型变体、上下文化双向转换和每变体测试。
...
逐变体测试保留表外属性样例，实际分类为 **58 个无损往返 + 9 个具名拒绝 = 67**。
...
`replaceText` 收 `{ from, to, text }`，位置线型与 `deleteRange` 相同，仅支持同段范围；
格式取首个被替换字符所属 run 的完整 `rPr`。空字符串表示删除，空范围表示普通插入。
它是一个原生事务操作，Agent `replaceText` 每处命中对应一条审计记录。详见 `spec/08` EDIT-03。

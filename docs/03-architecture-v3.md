...
| M8′ | 原生协议 `spec/21-bind.md`：会话与句柄、模型 JSON 投影、`EditOp` JSON、媒体句柄、`resolve` 查询；Rust crate 公共 API 定型；`*.model.json` 自快照回归网；`compat_ts` 降为测试专用 feature | 全语料 `document()` 过 JSON Schema 且 serde 往返幂等；公共 API 冻结并有文档；自快照进 CI；默认构建不含 `compat_ts` |

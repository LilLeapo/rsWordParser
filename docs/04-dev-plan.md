...
三条都已有测试覆盖（分支 `m1.15-diff-tools`，2026-09-04）：第一条 `cargo run -p diff-parse -- --scope text`（223 份文档 0 未知差异）与 `tests/compat.rs`；第二条 `tests/edit.rs::edit_03_insert_text_in_clean_run_keeps_everything_else`（其他条目 CRC 与字节相同，`SAVE-08` 的 `Clean` 子串自检在调试构建里对每个脏 part 抽样）；第三条 `tests/save_validate.rs::save_03_strict_document_stays_strict_after_edit`。

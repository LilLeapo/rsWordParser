...
| `spec/20` 9.0 形态实测 | B 的重载撤销栈、C 的片段输出阻碍需实测记录 | 当前工作树缺宿主版本、操作步骤和结果记录，本轮没有 Windows/Office.js 宿主复现；docs/12 区分初判与证据，列补证步骤 | A 已经批准，不重开形态选择；B/C 实测证据待补，9.0 不伪报全部验收完成 |
...
- [ ] **9.0 场景、验收集与预算基线**（`docs/12-agent-tasks.md`，草案与测量已交付，待评审）

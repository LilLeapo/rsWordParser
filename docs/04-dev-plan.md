...
- [x] **1.11 `EditSession` / `MutationPlan` 框架**（`edit/{mod,locate,plan}.rs`）：`EditSession::open(bytes)` 持有 `Package` + `Document` 投影，提供 `document()/package()/locate/plan/apply/apply_all/commit/add_media/save`（`EDIT-01`）；`EditContext` / `RevisionAuthor` / `EditOp`（M1 前四变体，具体 planner 在 1.12）/ `MediaId`；`locate` 按 `MOD-06` 坐标流实现 UTF-16 定位，`Loc::InText` 给段内字节偏移、`Loc::Boundary` 给展开的 segment 边界序号，代理对中间返回 `EDIT_SPLIT_SURROGATE`（`EDIT-02`）；`MutationPlan` 持有跨 part 的 `Vec<NodeEdit>` 与 `offset_delta`，`validate` 只读检查 part / 节点 / `Target::New` 引用 / `before` 父子关系，在任一写入前失败；`commit` 验证后 `Dom::apply_edits` → `declare_for_new_subtree` → `Document::rebuild`（M1 整量重建，增量为 M2）。`apply_all` 先完成整批 plan + validate 再开始 commit，任一失败整批不生效。验收：EDIT-02 两行（😀 代理对、PAGE 原子前后差 1）与 EDIT-05 失败回滚测试；`cargo fmt/clippy/test` 全绿。

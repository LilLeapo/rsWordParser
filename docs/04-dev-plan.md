...
> **2026-09-10 起**：L3 model 层已扁平化为单个 `crates/rsword/src/model/mod.rs`。本文各里程碑清单里写的
> `model/<名字>.rs`（`model/build.rs`、`model/table.rs`、`model/theme.rs` …）是任务完成当时的文件名，
> 保留以存记录；现在都在 `mod.rs` 内的对应区段里找。`spec/14`–`spec/19` 同理。
...
- [x] **1.7 分类规则表**（`model/mod.rs`）：R01–R07 为 `BodyClass::classify`，R08–R19 由 `ParagraphFacts::classify` 按私有规则方法表顺序判定，`ParagraphFacts::text_kind` 按 `MOD-03`（ListRef > Heading > Paragraph；直接 `outlineLvl 9` 不看样式；样式 `numId 0` 取消继承）。R12 的 ChartEx-Fallback-图 → Image 留到 M3。

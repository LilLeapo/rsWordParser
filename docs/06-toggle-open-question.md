...
### ~~第 2 件：把歧义频率做成常驻测量~~ —— **已完成（2026-09-08，`spec/18` 7.9）**

`tests/resolve.rs::res_04_toggle_ambiguity_probe`：遍历 `corpus/synthetic` 与 `corpus/real`，
数有多少 `(run, toggle 字段)` 落在歧义形状上。**判据收紧了一次**：run 自己的 `w:rPr` 声明了它
就不算歧义（直接格式在两条规则里都一票定音），真正的歧义是 run 自己没声明、而
`docDefaults` / 段落样式链 / 字符样式链里**两个及以上**声明了它。

**读数（2026-09-08）**：1,065 份文档、3,698 个 run，撞上歧义 13 次，全部来自我们自己为
`RES-04` 造的校准件 `toggle-other-toggles-converted`（它就是**故意**造成这个形状的），
**校准件之外 0 次**。探针不 fail，只打印；校准件之外非 0 时会另打一行提醒。

（下面是原来的计划，留着看判定阈值。）

### 第 2 件（原计划）：把歧义频率做成常驻测量
